...
{
    return HOME_SITE_URL.'/qrcode?url='. urlencode(config('h5_site_url').'/home/goodsdetail?goods_id='.$goods_info['goods_id']);
}
...
{
    return HOME_SITE_URL.'/qrcode?url='. urlencode(config('h5_site_url').'/home/storedetail?id='.$store_id);
}
 ...
 * @return string
 */
function goodsclass_image($cate_id = 0)
{
    if (file_exists(BASE_UPLOAD_PATH . DS . ATTACH_COMMON . DS . 'category-pic-'.$cate_id.'.jpg')) {
        return UPLOAD_SITE_URL . '/' . ATTACH_COMMON . '/' . 'category-pic-'.$cate_id.'.jpg';
    }
    return UPLOAD_SITE_URL . '/' . ATTACH_COMMON . '/default_goods_image.jpg';
}
/**
 * 取得订单状态文字输出形式
 *
