...
class Brand extends AdminControl {
    ...
    private function createExcel($data = array()) {
        ...
        $excel_obj->generateXML($excel_obj->charset(lang('exp_brand'), CHARSET) . input('param.curpage') . '-' . date('Y-m-d-H', TIMESTAMP));
    }
    ...
    protected function getAdminItemList() {
        $menu_array = array(
            ...
            array(
                'name' => 'brand_add',
                'text' => lang('ds_add'), 
                'url' => "javascript:dsLayerOpen('".url('Brand/brand_add')."','".lang('ds_add')."')"
            ),
            array(
                'name' => 'brand_apply', 
