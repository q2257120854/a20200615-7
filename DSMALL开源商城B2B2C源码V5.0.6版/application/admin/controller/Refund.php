...
class Refund extends AdminControl
{
    ...
    public function edit()
    {
        ...
        if (request()->isPost()) {
            ...
            if ($state) {
                ...
                // 发送微信模板消息
                $openid=db('member')->where(array('member_id'=>$refund['buyer_id']))->value('member_wxopenid');
                if($openid && config('wechat_tm_order_refund')){
                    $tm_data = array(
                        "first" => array(
                            "value" => lang('wechat_tm_order_refund'),
                            "color" => "#ff7007"
                        ),
                        "keyword1" => array(
                        ...
                        "remark" => array(
                            "value" => lang('order_refund_complete'),
                            "color" => "#333"
                        )
                    );
                    \mall\queue\QueueClient::push('sendWechatTemplateMessage', array(
                        'openid'=>$openid,
                        'template_id'=>config('wechat_tm_order_refund'),
                        'url'=>config('h5_site_url').'/member/refund_view?refund_id='.$refund['refund_id'],
                        'data'=>$tm_data,
                    ));
                }
    ...
    private function createExcel($data = array()) {
        ...
        $excel_obj->generateXML($excel_obj->charset(lang('exp_tk_refund'), CHARSET) . input('param.curpage') . '-' . date('Y-m-d-H', TIMESTAMP));
    }
    ...
    protected function getAdminItemList()
    {
        $menu_array = array(
            array(
                'name' => 'refund_manage', 'text' => lang('admin_state_2'), 'url' => url('Refund/refund_manage')
            ), array(
                'name' => 'refund_all', 'text' => lang('refund_all'), 'url' => url('Refund/refund_all')
            ), array(
