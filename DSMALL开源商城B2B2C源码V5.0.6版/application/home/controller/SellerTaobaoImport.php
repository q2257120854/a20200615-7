...
class SellerTaobaoImport extends BaseSeller {
            ...
            if (is_array($records) and count($records) > 0) {
                foreach ($records as $k => $record) {
                    if ($remain_num > 0 and $k >= $remain_num) {
                        $this->error(lang('store_goods_index_goods_limit') . $store_grade['sg_goods_limit'] . lang('store_goods_index_goods_limit1') . lang('store_goods_import_end') . (count($records) - $remain_num) . lang('store_goods_import_products_no_import'), url('SellerTaobaoImport/index', ['step' => 4]));
                    }
                ...
                if ($goods_id_str != "") {
                    $this->assign('goods_id_str', substr($goods_id_str, 1, strlen($goods_id_str)));
                }
            }
            $this->assign('step', '4');
        }
    ...
    public function import_image() {
        return $this->fetch($this->template_dir . 'import_image');
    }

    public function upload() {
        if (isset($_FILES["Filedata"]) || !is_uploaded_file($_FILES["Filedata"]["tmp_name"]) || $_FILES["Filedata"]["error"] != 0) {
            $store_id = session('store_id');
            $path = BASE_UPLOAD_PATH . DS . ATTACH_GOODS . DS . $store_id; //取得上传图片的绝对路径
            $SID = $store_id . "_";
            if (!is_dir($path)) {
                mkdir($path, 0777);
            }//如果目录不存在，则创建
            ...
            foreach ($record as $k => $col) {
                ...
                switch ($k) {
                    case $fields_cols['goods_body'] : $return[$key]['goods_body'] = str_replace('\"\"', '"', $col);
                        ...
                        $url = UPLOAD_SITE_URL."/home/store/goods/".session('store_id')."/".session('store_id')."_";
                        $returngoodsbodyurl[0]=str_replace('\\', '/', $returngoodsbodyurl[0]);
                        $return[$key]['goods_body']=str_replace('\\', '/', $return[$key]['goods_body']);
