...
class Sellerpromotionbooth extends BaseSeller
{
    ...
    public function index()
    {
        ...
        if (!$isPlatformStore) {
            ...
            if (!empty($booth_quota)) {
                $hasList = true;
            }elseif(intval(config('promotion_booth_price'))==0){
                $hasList = true;
            }else{
    ...
    {
        if (request()->isPost()) {
            if(intval(config('promotion_booth_price'))==0){
    ...
    public function choosed_goods()
    {
        ...
        if (count($bootgoods_info) >= config('promotion_booth_goods_sum')) {
            $data = array('result' => 'false', 'msg' => sprintf(lang('promotion_booth_goods_sum_error'),config('promotion_booth_goods_sum')));
            $this->_echoJson($data);
        }
        // 商品是否已经被添加
            ...
            case 'booth_renew':
                $menu_array = array(
                    array(
                        ...
                        'name' => 'booth_renew', 'text' => lang('subscription_fee'), 'url' => url('Sellerpromotionbooth/booth_renew')
                    )
                );
                break;
        }
