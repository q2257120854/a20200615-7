...
class TaobaoExport extends BaseGoods {
    ...
    public function _initialize() {
        Lang::load(APP_PATH . 'home/lang/' . config('default_lang') . '/taobao_export.lang.php');
        parent::_initialize();
    }
    ...
    public function export_image(){
        ...
        $exportPath = BASE_UPLOAD_PATH . DS . ATTACH_TAOBAO . DS . 'goods_image_' . $goods_info['goods_id'];
        if (!file_exists($exportPath.'.zip')) {
            if (!is_dir($exportPath)) {
                mkdir($exportPath, 0777, true);
            }//如果目录不存在，则创建
            //图片
            $image_path_1=iconv("UTF-8", "GBK", $exportPath . DS . '主图');
            if (!is_dir($image_path_1)) {
                mkdir($image_path_1, 0777);
            }//如果目录不存在，则创建
            $image_path_2=iconv("UTF-8", "GBK", $exportPath . DS . '详情图');
            if (!is_dir($image_path_2)) {
                mkdir($image_path_2, 0777);
            }//如果目录不存在，则创建
            foreach ($goods_detail['goods_image'] as $key_gi => $val_gi) {
                @file_put_contents($image_path_1 . DS . basename($val_gi[2]), file_get_contents($val_gi[2]));
            }
            $goods_body=htmlspecialchars_decode($goods_info['goods_body']);
            if(preg_match_all("/<\s*img\s+[^>]*?src\s*=\s*(\'|\")(.*?)\\1[^>]*?\/?\s*>/i",$goods_body,$matches)){
                foreach($matches[2] as $match){
                    @file_put_contents($image_path_2 . DS . basename($match), file_get_contents($match));
                }
            }
    ...
    public function export_csv() {
        ...
        $exportPath = BASE_UPLOAD_PATH . DS . ATTACH_TAOBAO . DS . 'goods_csv_' . $goods_info['goods_id'];
        if (!file_exists($exportPath.'.zip')) {
            if (!is_dir($exportPath)) {
                mkdir($exportPath, 0777, true);
            }//如果目录不存在，则创建
...
";
            ...
            //图片
            if (!is_dir($exportPath . DS . 'goods_' . $goods_info['goods_id'])) {
                mkdir($exportPath . DS . 'goods_' . $goods_info['goods_id'], 0777);
            }//如果目录不存在，则创建
            $goods_image = "";
            foreach ($goods_detail['goods_image'] as $key_gi => $val_gi) {
                $temp = explode('.', basename($val_gi[2]));
                @file_put_contents($exportPath . DS . 'goods_' . $goods_info['goods_id'] . DS . $temp[0] . '.tbi', file_get_contents($val_gi[2]));
            ...
            file_put_contents($exportPath . DS . 'goods_' . $goods_info['goods_id'] . '.csv', $str);
