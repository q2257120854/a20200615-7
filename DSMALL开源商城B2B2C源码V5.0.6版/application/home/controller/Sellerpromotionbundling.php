...
class Sellerpromotionbundling extends BaseSeller
{
    ...
    public function index()
    {
        ...
        if (!$isPlatformStore) {
            ...
            if (!empty($bundling_quota)) {
                ...
                $hasList = true;
            }elseif(intval(config('promotion_bundling_price'))==0){//如果没有购买过套餐，则将之前添加的优惠组合关闭
                $hasList = true;
            }else{
        ...
        if (request()->isPost()) {
            ...
            if (intval(input('param.bundling_id')) > 0) {
                ...
                if (empty($bundling_info['store_id'])) {
                    $this->error(lang('param_error'), url('Sellerpromotionbundling/index'));
                }
    ...
    public function bundling_add_goods()
    {
        ...
        // where条件
        $where = array();
        $where['goods.store_id'] = session('store_id');
        if (intval(input('param.storegc_id')) > 0) {
            $where['goods.goods_stcids'] = array('like', '%,' . intval(input('param.storegc_id')) . ',%');
        }
        if (trim(input('param.keyword')) != '') {
            $where['goods.goods_name'] = array('like', '%' . trim(input('param.keyword')) . '%');
        }
        ...
        echo $this->fetch($this->template_dir . 'bundling_add_goods');exit;
    }
