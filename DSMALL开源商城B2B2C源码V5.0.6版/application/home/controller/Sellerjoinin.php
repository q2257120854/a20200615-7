            ...
            switch (intval($joinin_detail['joinin_state'])) {
                ...
                case STORE_JOIN_STATE_VERIFY_FAIL:
                    if (!in_array(request()->action(), array('step1', 'step2', 'step3', 'step4'))) {
                        $this->show_join_message(lang('verify_fail').':' . $joinin_detail['joinin_message'], url('Sellerjoinin/step1'));
                    }
                    break;
                case STORE_JOIN_STATE_PAY_FAIL:
                    if (!in_array(request()->action(), array('pay', 'pay_save'))) {
                        $this->show_join_message(lang('pay_verify_fail').':' . $joinin_detail['joinin_message'],url('Sellerjoinin/pay') );
                    }
                    break;
                case STORE_JOIN_STATE_FINAL:
                    $this->success(lang('store_already_open'),url('Sellerlogin/login'));
                    break;
            }
        }
    }
