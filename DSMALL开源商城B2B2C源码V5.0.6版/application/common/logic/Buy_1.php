...
class Buy_1 extends Model
{
    ...
    public function getBargainInfo(& $goods_info, $quantity, $extra) {
        ...
        //之前是否已添加过订单
        if(model('order')->getOrdergoodsInfo(array('promotions_id'=>$pbargainorder_info['bargainorder_id']))){
            return;
        }
        //购买数量是否超过限制数量
    ...
    public function getPintuanInfo(& $goods_info, $quantity,$extra,$member_id){
        ...
        if($pintuangroup_id>0){
            ...
            //不可以重复参加
            $order_id_list=db('ppintuanorder')->where(array('pintuan_id'=>$goods_info['pintuan_info']['pintuan_id'],'pintuanorder_state'=>array('<>',0)))->column('order_id');
            if($order_id_list){
