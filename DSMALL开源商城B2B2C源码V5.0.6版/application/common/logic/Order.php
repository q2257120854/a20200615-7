...
class Order extends Model
{
    ...
    public function changeOrderSend($order_info, $role, $user = '', $post = array())
    {
        ...
        if ($post['shipping_express_id'] && isset($post['shipping_code'])) {
            ...
            if ($openid && config('wechat_tm_order_send')) {
                $tm_data = array(
                    "first" => array(
                        "value" => lang('wechat_tm_order_send'),
                        "color" => "#ff7007"
                    ),
                    "keyword1" => array(
                        "value" => $post['shipping_code'],
                        "color" => "#333"
                    ),
                    "keyword2" => array(
                    ...
                    "keyword4" => array(
                        "value" => $order_info['extend_order_common']['reciver_name'],
                        "color" => "#333"
                    ),
                    "keyword5" => array(
                        "value" => $order_info['extend_order_common']['reciver_info']['address'],
                        "color" => "#333"
                    ),
                    "remark" => array(
                        "value" => lang('order_state_send_notice'),
                        "color" => "#333"
                    )
                );
                \mall\queue\QueueClient::push('sendWechatTemplateMessage', array(
                    'openid' => $openid,
                    'template_id' => config('wechat_tm_order_send'),
