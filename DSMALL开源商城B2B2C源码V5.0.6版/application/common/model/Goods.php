...
class Goods extends Model {
    ...
    public function getGoodsList($condition, $field = '*', $group = '', $order = '', $limit = 0, $pagesize = 0, $lock = false, $count = 0) {
        ...
        if ($pagesize) {
            ...
            $result=$result->order($order)->paginate($pagesize,false,['query' => request()->param()]);
    ...
    public function getGoodsUnionList($condition, $field, $order = 'goodscommon.mall_goods_commend desc,goodscommon.mall_goods_sort asc',$group='', $pagesize = 0,$limit=0) {
        $condition['goodscommon.goods_state'] = self::STATE1;
        $condition['goodscommon.goods_verify'] = self::VERIFY1;
        if(isset($condition['goodscommon.gc_id'])){
            $condition = $this->_getRecursiveClass($condition,'goodscommon.');
        }

        $result=db('goodscommon')->alias('goodscommon')->join('__GOODS__ goods','goods.goods_commonid=goodscommon.goods_commonid')->field($field)->where($condition)->order($order);
        if($group){
            $result=$result->group($group);
        }
        if ($pagesize) {
            $result=$result->paginate($pagesize,false,['query' => request()->param()]);
            $this->page_info = $result;
            return $result->items();
        } else {
    ...
    public function getGoodsListForPromotion($condition, $field = '*', $pagesize = 0, $type = '') {
        switch ($type) {
            ...
            case 'bargain':
            case 'bundling':
                $condition['goodscommon.is_virtual'] = 0;
                $condition['goodscommon.is_goodsfcode'] = 0;
                $condition['goodscommon.is_presell'] = 0;
                $condition['goodscommon.goods_lock'] = 0;
                $condition['goodscommon.goods_state'] = self::STATE1;
                $condition['goodscommon.goods_verify'] = self::VERIFY1;
    ...
    public function getGoodsCommonList($condition, $field = '*', $pagesize = 10, $order = 'goods_commonid desc') {
        $condition = $this->_getRecursiveClass($condition);
        if ($pagesize) {
            $result = db('goodscommon')->field($field)->where($condition)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
            $this->page_info = $result;
            return $result->items();
        } else {
    ...
    public function getGoodsInfoAndPromotionById($goods_id) {
        ...
        //会员等级折扣
        if (empty($goods_info['bargain_info']) && empty($goods_info['groupbuy_info']) && empty($goods_info['pintuan_info']) && empty($goods_info['xianshi_info'])) {
    ...
    public function editGoodsImages($update, $condition) {
        $result = db('goodsimages')->where($condition)->update($update);
        return $result;
    }
    /**
     * 删除商品图片表信息
     ...
     * @return array
     */
    private function _getRecursiveClass($condition,$prefix='') {
        if (isset($condition[$prefix.'gc_id']) && !is_array($condition[$prefix.'gc_id'])) {
            $gc_list = model('goodsclass')->getGoodsclassForCacheModel();
            if (!empty($gc_list[$condition[$prefix.'gc_id']])) {
                $gc_id[] = $condition[$prefix.'gc_id'];
                $gcchild_id = empty($gc_list[$condition[$prefix.'gc_id']]['child']) ? array() : explode(',', $gc_list[$condition[$prefix.'gc_id']]['child']);
                $gcchildchild_id = empty($gc_list[$condition[$prefix.'gc_id']]['childchild']) ? array() : explode(',', $gc_list[$condition[$prefix.'gc_id']]['childchild']);
                $gc_id = array_merge($gc_id, $gcchild_id, $gcchildchild_id);
                $condition[$prefix.'gc_id'] = array('in', $gc_id);
            }
        }
        return $condition;
    }
