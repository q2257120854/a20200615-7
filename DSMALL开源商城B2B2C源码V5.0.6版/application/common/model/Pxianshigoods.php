...
class Pxianshigoods extends Model {
    ...
    public function getXianshigoodsList($condition, $pagesize = null, $order = '', $field = '*', $limit = 0) {
        if ($pagesize) {
            $res = db('pxianshigoods')->field($field)->where($condition)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
            $this->page_info = $res;
            return $res->items();
        } else {
    ...
    public function addXianshigoods($xianshigoods_info) {
        ...
        if($xianshigoods_id){
            // 发布限时锁定商品
            $this->_lockGoods($xianshigoods_info['goods_commonid']);
        }
        // 删除商品限时折扣缓存
    ...
    public function delXianshigoods($condition) {
        ...
        if ($result) {
            if (!empty($xianshigoods_list)) {
                foreach ($xianshigoods_list as $val) {
                    $this->_unlockGoods($val['goods_commonid']);
                    // 删除商品限时折扣缓存
                    $this->_dGoodsXianshiCache($val['goods_id']);
                    // 插入对列 更新促销价格
                    \mall\queue\QueueClient::push('updateGoodsPromotionPriceByGoodsId', $val['goods_id']);
    ...
    public function getXianshigoodsExtendInfo($xianshi_info) {
        $xianshi_info['goods_url'] = url('/Home/Goods/index', array('goods_id' => $xianshi_info['goods_id']));
