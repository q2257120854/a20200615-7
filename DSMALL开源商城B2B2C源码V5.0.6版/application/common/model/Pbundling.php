...
class Pbundling extends Model
{
    ...
    {
        if ($pagesize) {
            $res = db('pbundling')->where($condition)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
            $this->page_info=$res;
            return $res->items();
        }
        else {
    ...
    {
        if ($pagesize) {
            $res = db('pbundlingquota')->where($condition)->order('blquota_id desc')->paginate($pagesize,false,['query' => request()->param()]);
            $this->page_info = $res;
            return $res->items();
        }
        else {
    ...
    public function addBundlingGoodsAll($data)
    {
        ...
        if ($result) {
            foreach ((array)$data as $v) {
                // 发布套餐锁定商品
                $this->_lockGoods($v['goods_commonid']);
    ...
    public function delBundlingGoods($condition)
    {
        ...
        if ($result) {
            foreach ($list as $v) {
                $this->_unlockGoods($v['goods_commonid']);
                $this->_dGoodsBundlingCache($v['goods_id']);
            }
            return true;
        }
