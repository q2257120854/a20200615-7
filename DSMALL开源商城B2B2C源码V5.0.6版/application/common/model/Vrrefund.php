...
class Vrrefund extends Model {
     ...
     */
        ...
        public function getVrrefundList($condition = array(), $pagesize = '',  $field = '*', $order = 'refund_id desc', $limit = '') {
        if($pagesize){
            $result = db('vrrefund')->field($field)->where($condition)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
