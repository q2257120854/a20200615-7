...
class Adv extends Model {
    ...
    public function getAdvpositionList($condition = array(), $pagesize = '', $orderby = 'ap_id desc') {
        if ($pagesize) {
            $result = db('advposition')->where($condition)->order($orderby)->paginate($pagesize, false, ['query' => request()->param()]);
     ...
     * @return array 二维数组
     */
    public function getAdvList($condition = array(), $pagesize = '', $limit = '', $orderby = 'adv_id desc') {
        if ($pagesize) {
            $result = db('adv')->where($condition)->order($orderby)->paginate($pagesize, false, ['query' => request()->param()]);
            $this->page_info = $result;
            return $result->items();
        } else {
