...
class Activitydetail extends Model
{
    ...
    public function getActivitydetailList($condition,$pagesize='',$order='activitydetail_sort desc'){
        if ($pagesize) {
            $res = db('activitydetail')->where($condition)->order($order)->paginate($pagesize, false, ['query' => request()->param()]);
            $this->page_info = $res;
            return $res->items();
        }else{
    ...
    public function getGoodsJoinList($condition,$pagesize='',$order=''){
        ...
        $res=db('activitydetail')->alias('activitydetail')->join('__GOODS__ goods','activitydetail.item_id=goods.goods_id')->field($field)->where($condition)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
        $this->page_info=$res;
        return $res->items();
    }
    /**
