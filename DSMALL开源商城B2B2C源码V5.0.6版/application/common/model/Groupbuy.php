...
class Groupbuy extends Model
{
    ...
    {
        $result = db('groupbuy')->alias('b')->field($field)->join('__GOODS__ g', 'b.goods_id=g.goods_id', 'RIGHT')->where($condition)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
    ...
    {
        $result= db('groupbuy')->field($field)->where($condition)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
        $this->page_info=$result;
        return $result->items();
    }
