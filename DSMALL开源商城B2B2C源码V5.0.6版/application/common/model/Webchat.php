...
class Webchat extends Model
{
    ...
    {
        $list = db('chatlog')->where($condition)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
    ...
    {
        $list = db('snsfriend')->where($condition)->order('friend_addtime desc')->paginate($limit,false,['query' => request()->param()]);
        if ($list) {
            foreach ($list as $k => $v) {
                $member = array();
                $u_id = $v['friend_tomid'];
     ...
     * @return type
     */
    public function getMemberRecentList($condition = array(), $pagesize = '', $limit = '')
    {
        $list = array();
        ...
        if ($msg['count'] > 0) {
            ...
            $list = db('chatmsg')->field($field)->group('t_id')->where($condition)->limit($limit)->page($pagesize, $count)->order('addtime desc')->select();
        }
        return $list;
    }
     ...
     * @return type
     */
    public function getMemberFromList($condition = array(), $pagesize = '', $limit = '')
    {
        $list = array();
        ...
        if ($msg['count'] > 0) {
            ...
            $list = db('chatmsg')->field($field)->group('f_id')->where($condition)->limit($limit)->page($pagesize, $count)->order('addtime desc')->select();
        }
        return $list;
    }
     ...
     * @return type
     */
    public function getChatlogFromList($condition = array(), $pagesize = 10, $order = 'm_id desc')
    {
        $list = array();
