...
class Storedepositlog extends Model {
    ...
    public function changeStoredeposit($data = array()) {
        ...
        $store_info=db('store')->where('store_id',$data['store_id'])->field('store_avaliable_deposit,store_freeze_deposit,store_payable_deposit,store_name')->lock(true)->find();
        if(!$store_info){
        ...
        if(!empty($store_data)){
            if(!db('store')->where('store_id',$data['store_id'])->update($store_data)){
