...
class Ppintuanorder extends Model {
    ...
    public function getPpintuanvrorderList($condition) {
        $ppintuanorder_list = db('ppintuanorder')->alias('ppintuanorder')->field('ppintuanorder.*,vrorder.buyer_id,vrorder.buyer_name,vrorder.order_state')
                ->join('__VRORDER__ vrorder', 'ppintuanorder.order_id=vrorder.order_id','LEFT')
                ->where($condition)
                ->select();
        if (!empty($ppintuanorder_list)) {
            foreach ($ppintuanorder_list as $key => $ppintuanorder) {
