...
class Pbargain extends Model {
    ...
    public function getBargainList($condition, $pagesize = null, $order = 'bargain_id desc', $field = '*') {
        $res = db('pbargain')->field($field)->where($condition)->order($order)->paginate($pagesize, false, ['query' => request()->param()]);
    ...
    public function getOnlineBargainList($condition, $pagesize = null, $order = 'bargain_id desc', $field = '*') {
        $condition['bargain_state'] = self::PINTUAN_STATE_NORMAL;
    ...
    public function getBargainInfo($condition) {
        $bargain_info = db('pbargain')->where($condition)->find();
        return $bargain_info;
    }
    ...
    public function addBargain($data) {
        $flag= db('pbargain')->insertGetId($data);
        if($flag){
            // 发布砍价锁定商品
            $this->_lockGoods($data['bargain_goods_commonid']);
        }
        return $flag;
    }
    ...
    public function editBargain($update, $condition) {
        return db('pbargain')->where($condition)->update($update);
    }
    ...
    public function endBargain($condition=array()) {
        $condition['bargain_state'] = self::PINTUAN_STATE_NORMAL;
        if(isset($condition['bargain_goods_commonid'])){
            $goods_commonid=$condition['bargain_goods_commonid'];
        }else{
            $goods_commonid=db('pbargain')->where($condition)->column('bargain_goods_commonid');
        }
        $data['bargain_state'] = self::PINTUAN_STATE_END;
        $flag= db('pbargain')->where($condition)->update($data);
        if($flag){
            if(!empty($goods_commonid)){
                $this->_unlockGoods(is_array($goods_commonid)?array('in',$goods_commonid):$goods_commonid);
            }
        }
        return $flag;
    }
