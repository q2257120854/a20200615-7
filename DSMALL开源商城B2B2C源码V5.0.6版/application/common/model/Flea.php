...
class Flea extends Model
{
     ...
     * @return type
     */
    public function getFleaList($condition, $pagesize = '', $field = '*',$order = 'goods_id desc')
    {
        $where = $this->getCondition($condition);
        if ($pagesize) {
            $res = db('flea')->alias('flea')->where($where)->field($field)->order($order)->paginate($pagesize, false, ['query' => request()->param()]);
            $this->page_info = $res;
    ...
    {
        if($pagesize){
            $member_list = db('fleaupload')->where($condition)->paginate($pagesize,false,['query' => request()->param()]);
