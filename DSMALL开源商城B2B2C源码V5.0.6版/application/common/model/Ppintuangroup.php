...
class Ppintuangroup extends Model {
    ...
    public function getPpintuangroupList($condition, $pagesize = '',$order='pintuangroup_starttime desc') {
        ...
        if ($pagesize) {
            $result = db('ppintuangroup')->alias('ppintuangroup')->join('__MEMBER__ member','ppintuangroup.pintuangroup_headid=member.member_id')->field($field)->where($condition)->order($order)->paginate($pagesize, false, ['query' => request()->param()]);
            $this->page_info = $result;
            $ppintuangroup_list = $result->items();
        }else{
            $ppintuangroup_list =  db('ppintuangroup')->alias('ppintuangroup')->join('__MEMBER__ member','ppintuangroup.pintuangroup_headid=member.member_id')->field($field)->where($condition)->order($order)->select();
        }
