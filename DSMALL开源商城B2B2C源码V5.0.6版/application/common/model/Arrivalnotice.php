...
class Arrivalnotice extends Model
{
     ...
     * @return array
     */
    public function getArrivalNoticeList($condition = array(), $field = '*', $limit = '', $order = 'arrivalnotice_id desc',$pagesize = '') {
        if ($pagesize) {
            $result = db('arrivalnotice')->where($condition)->order($order)->paginate($pagesize, false, ['query' => request()->param()]);
