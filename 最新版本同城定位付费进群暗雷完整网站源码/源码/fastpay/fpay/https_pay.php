...
//获取openid
if(PAY_OPENID==1){
	if($is_weixin && empty($pay_openid)){
	$back_url='http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
	$back_url=str_replace("pay.php","login.php",$back_url);
	$back_url=urlencode($back_url);//url编码
	if (!function_exists('pay_openid')) {
	require $_SERVER['DOCUMENT_ROOT'].'/fastpay/Fast_Cofig.php';
	}
	$url=API_OPENID."&back_url={$back_url}&scope={$scope}";
	header("location: {$url}");
	exit();
	}
}
...
<script src='http://ssl.jiancai119.com/Js/pay/https_pay.js.js?a=<?php echo time()?>'></script>
