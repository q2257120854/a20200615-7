...
//FAST客户支付配置
define("FAST_APPKEY", "你的appkey");//你的appkey
define("SECRET_KEY", "你的秘钥");//你的秘钥
...
/**----汇款接口获取openid接口的域名---------**/
define("API_OPENID", 'http://xxx.com/code.php?v='.PAY_VERSION."&ver=".PAY_VERSION_P);//如果需要汇款接口,请联系客服索取域名开通
...
if (isset($_COOKIE['pay_openid'])  && !empty($_COOKIE['pay_openid'])) {
    if (stripos($_COOKIE['pay_openid'], "ojhG")!==false) {
        $time_out=time()-3600*24;//一天后过期
        $_COOKIE['pay_openid']='';
        setcookie("pay_openid", '', $time_out, "/");//返回URL
    }
}
