...
file_put_contents('./aaaaccc.txt',json_encode($_POST));
include('../system/inc.php');	
$data['money'] = $_REQUEST['money'];            	
$ddh	    =	$_REQUEST['out_trade_no'];  //接收交易号这个是支付宝财付通微信的 官方流水账号   能用到
$Money2		=	$_REQUEST['money'];
$money     =   $_REQUEST['money'];//订单金额  能用到
$dingdan		=	$_REQUEST['out_trade_no']; //自己网站上订单的订单号  能用到
  ...
  function kouliangorder($uid)
{
	...
	if (!!($row = @mysql_fetch_array($result))) {
	  ...
	  $results = mysql_query('select * from '.flag.'order where  dingdanhao="'.$dingdan.'"  and zt=0  ');
...
/* if($rows['uid'] ==90){
	    file_put_contents('./aaa.txt', $weikouliangnum.'=='.$shijizhi);
	}*/
   //满足扣量条件
	...
	{   $kouliang=0;}	 
		/*if($rows['uid'] ==90){
	    file_put_contents('./bbb.txt', $kouliang.'=='.$jixukou);
	}*/
...
$sql = 'update '.flag.'order set jiaoyihao=  "'.$ddh.'",zt=1,payprice='.$shijidashangje.',pdate="'.date('Y-m-d H:i:s').'"   where dingdanhao="'.$dingdan.'"  ';
  mysql_query($sql);
  $sql11 = 'update '.flag.'baoy set zt=1,time="'.time().'" where ddh="'.$dingdan.'"  ';
  mysql_query($sql11);
