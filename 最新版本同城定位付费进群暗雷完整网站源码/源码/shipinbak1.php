...
$row = mysql_fetch_array($result);
{
  ...
  $result = mysql_query('select * from '.flag.'usershipin where id > '.$_GET['id'].' and uid = '.$uid.' order by id desc ');
...
{$sid='';}
 ...
 {$zf=0; $payurl='/'.$url.'.php?id='.$_GET['id'].''; $play='<img src="'.$image.'" alt="">';
  // alert_url('/'.$url.'.php?id='.$_GET['id'].''); 
}
else

{
$zf=0; $payurl='';	$play='<div class="rich_media_content"  id="video" style="width: 100%; height: 250px;border: solid 1px #323136;padding-left:1px;"></div>';
...
<title>รางวัลเมฆแท้</title>
  ...
  <!-- Link Swiper's CSS -->
  <link rel="stylesheet" href="/template/02/agency/css/swiper.min.css">
  <link rel="stylesheet" href="/template/02/agency/css/animate.min.css">
...
<?=$play?>
            ...
            <div class="info-box">
                <p class="user_name">รางวัลเมฆแท้</p>
                 <? if ($site_suiji==1) {$dashangsls=rand(1000,9999);} else {$dashangsls=get_ordersl($_GET['id']);} ?>
                <p class="user_tag">已有<?=$dashangsls?>人付费观看，好评度：99.5%</p>
            </div>
        </div>
    </div>
  </div>
<div class="footer">
  <div class="footer_wrap">
      <span class="swiper-button-prev prev_div"><a href="<?=$sid?>"><img src="/template/02/agency/images/left.png" alt=""></a></span>
      <span class="mid_div"><a href="<?=$payurl?>"><img src="/template/02/agency/images/play.png" alt=""></a></span>
      <span class="swiper-button-next next_div"><a href="<?=$xid?>"><img src="/template/02/agency/images/right.png" alt=""></a></span>
  </div>

</div>
  <script src="/template/02/agency/js/flexible.js"></script>
  <script src="/template/02/agency/js/swiper.min.js"></script>
  <script src="/template/02/agency/js/swiper.animate.min.js"></script>
