...
document.querySelectorAll("#标题栏1 a")[0].innerHTML += "<h1 style='font-size: 15px;float: right;color:" + 激活颜色 + ";'>我的收藏</h1>";
document.querySelectorAll("#标题栏1 a")[1].innerHTML = "<h1 style='font-size: 15px;float: right;color:" + 激活颜色 + "'>编辑</h1>";
function Shoucang_创建完毕(){
	if (sqlite数据库1.打开数据库("mydatabase1",1024*1024) == true){
		if (读写设置.读取设置("数据表") != "已创建" ){
			sqlite数据库1.创建数据表("mytable1","liebiao text, laiyuan text, biaoti text, tupian text, dizhi text, shijian integer");
		}
		console.log("打开数据库成功");
	}else{
		console.log("打开数据库失败");
	...
	sqlite数据库1.查询数据("mytable1","liebiao='liebiao'");
...
function 跳转播放(index){
	if (是否开启 == false){
		...
		读写设置.保存设置("视频剧集",视频总时间[index]);
		公用模块.影视播放(视频标题[index],视频地址[index]);
	}
	if (是否开启 == true){
		sqlite数据库1.删除数据("mytable1","biaoti='" + 视频标题[index] + "'");
		sqlite数据库1.查询数据("mytable1","liebiao='liebiao'");
	}
}
...
function 开始删除(i){
	...
	if (i == 1){
		sqlite数据库1.删除数据("mytable1","liebiao='liebiao'");
		sqlite数据库1.查询数据("mytable1","liebiao='liebiao'");
	}
}