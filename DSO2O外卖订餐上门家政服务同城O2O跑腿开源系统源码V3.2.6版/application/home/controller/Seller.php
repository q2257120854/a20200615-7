...
class  Seller extends BaseSeller {
    ...
    public function index() {
        ...
        // 昨日销量
        $where['order_add_time'] = array('between', array(strtotime(date('Y-m-d', (TIMESTAMP - 3600 * 24))), strtotime(date('Y-m-d', TIMESTAMP)) - 1));
        $daily_sales = model('stat')->getoneByStatorder($where, $field);
        $this->assign('daily_sales', $daily_sales);
        // 月销量
        $where['order_add_time'] = array('gt', strtotime(date('Y-m', TIMESTAMP)));
