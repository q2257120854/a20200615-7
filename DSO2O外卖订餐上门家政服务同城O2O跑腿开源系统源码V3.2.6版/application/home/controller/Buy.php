...
class  Buy extends BaseMember {
    ...
    public function buy_step1() {
        ...
        $this->assign('store_ids', implode(',', array_keys($result['store_cart_list'])));
        //商品金额计算(分别对每个商品/优惠套装小计、每个店铺小计)
        $this->assign('store_cart_list', $result['store_cart_list']);
        $this->assign('store_goods_total', $result['store_goods_total']);
        $this->assign('store_o2o_fee_list', $result['store_o2o_fee_list']);
...
//        $this->assign('pd_amount', ds_price_format($pd_amount));
    ...
    public function o2o_errand_pay(){
        ...
        $this->assign('pay_info', $result['data']);
        $this->assign('payment_list', $payment_list);

        return $this->fetch($this->template_dir.'pay');
    }
    /**
     * 服务订单支付页面
     */
    ...
    function calc_distance(){
        ...
        if(!input('param.store_ids')){
            exit(json_encode(array('state'=>false,'msg'=>lang('param_error'))));
        }
        $store_list=db('store')->where('store_id','in',input('param.store_ids'))->field('store_id,store_name,store_longitude,store_latitude,store_o2o_open,store_o2o_support,store_o2o_distribute_type,store_o2o_distance,store_o2o_fee,is_platform_store')->select();
        $res=$buy_1_logic->calcDistance(input('param.addr_id'),$store_list);
