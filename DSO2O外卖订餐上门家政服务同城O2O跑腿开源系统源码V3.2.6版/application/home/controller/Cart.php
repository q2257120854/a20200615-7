...
//            $logic_buy_1->getXianshiInfo($goods_info, $quantity);
    ...
    public function del() {
        ...
        if (session('member_id')) {
            //登录状态下删除数据库内容
            $delete = $cart_model->delCart('db', array('cart_id' => array('in',$cart_id), 'buyer_id' => session('member_id')));
            if ($delete) {
                $data['state'] = 'true';
            ...
            //未登录时删除cookie的购物车信息
            $delete = $cart_model->delCart('cookie', array('goods_id' => array('in',$cart_id)));
            if ($delete) {
                $data['state'] = 'true';
                $data['quantity'] = $cart_model->cart_goods_num;
                $data['amount'] = $cart_model->cart_all_price;
