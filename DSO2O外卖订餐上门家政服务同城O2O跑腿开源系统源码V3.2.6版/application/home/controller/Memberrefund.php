...
class  Memberrefund extends BaseMember {
    ...
    public function add_refund() {
        ...
        if (request()->isPost() && $goods_id > 0) {
            ...
            if ($state) {
                // 发送微信模板消息
                $member_id=db('store')->where(array('store_id'=>$order['store_id']))->value('member_id');
                ...
                if($openid && config('wechat_tm_order_refund')){
                    ...
                    \mall\queue\QueueClient::push('sendWechatTemplateMessage', array(
                        ...
                        'url'=>config('h5_site_url').'/seller/refund_view?refund_id='.$state,
                        'data'=>$tm_data,
                    ));
                }
                if ($order['order_state'] == $order_shipped) {
