...
class  MemberO2oComplaint extends BaseMember {
    ...
    public function _initialize() {
        parent::_initialize(); 
        Lang::load(APP_PATH . 'home/lang/'.config('default_lang').'/member_o2o_complaint.lang.php');
    }
    ...
    public function index() {
        ...
        if($select_complain_state!==null && $select_complain_state!==''){
            $condition['o2o_complaint_state'] = intval($select_complain_state);
        }
        ...
        $this->assign('show_page', $o2o_complaint_model->page_info);
    ...
    public function add() {
        ...
        $order_info = $order_model->getOrderInfo(array('order_id'=>$order_id,'buyer_id'=>session('member_id'),'finnshed_time'=>array('>',0),'order_state'=>ORDER_STATE_SUCCESS));
        if (!$order_info) {
    ...
    public function del(){
        ...
        if($o2o_complaint_model->delO2oComplaint(array('member_id'=>session('member_id'),'o2o_order_bill_id'=>0))){
            ds_json_encode(10000,lang('ds_common_op_succ'));
        }else{
