...
class MemberO2oErrandOrder extends BaseMember {
    ...
    public function _initialize() {
        parent::_initialize();
        Lang::load(APP_PATH . 'home/lang/' . config('default_lang') . '/member_o2o_errand_order.lang.php');
    }
    ...
    public function index() {
        ...
        if ($o2o_errand_order_state !== null && $o2o_errand_order_state !== '') {
            $condition['o2o_errand_order_state'] = intval($o2o_errand_order_state);
        }
        ...
        if($start_unixtime || $end_unixtime){
            $condition['o2o_errand_order_add_time'] = array('between', array($start_unixtime?$start_unixtime:0, $end_unixtime?($end_unixtime+86399):TIMESTAMP));
        }
        ...
        $this->assign('order_list', $o2o_errand_order_list);
        $this->assign('show_page', $o2o_errand_order_model->page_info->render());
    ...
    public function view() {
        ...
        $condition = array();
        $condition['member_id'] = $this->member_info['member_id'];
        $condition['o2o_errand_order_id'] = $o2o_errand_order_id;
        ...
        $this->assign('o2o_errand_order_info', $o2o_errand_order_info);
        $this->setMemberCurMenu('member_o2o_errand_order');
    ...
    public function send() {
        ...
        $this->assign('address_send_info', $address_send_info);
        //获取默认收件信息
        ...
        $this->assign('address_receive_info', $address_receive_info);
    ...
    public function add() {
        ...
        //计算距离费用
        $distance_price_list = unserialize(config('o2o_errand_distance_price'));
        ...
        //计算重量费用
        $weight_price_list = unserialize(config('o2o_errand_weight_price'));
        ...
        $time = $time / 60;
        $time_price_list = unserialize(config('o2o_errand_time_price'));
        ...
        $o2o_errand_order_validate = validate('o2o_errand_order');
    ...
    public function receive() {
        $order_id = intval(input('post.o2o_errand_order_id'));
        if (!$order_id) {
            ds_json_encode(10001, 'Hacking Attempt');
        }

        $order_model = model('o2o_errand_order');
        $conditions=array('member_id' => $this->member_info['member_id'],'o2o_errand_order_state' => ORDER_STATE_SEND,'o2o_errand_order_id' => $order_id);
        $o2o_errand_order_info = $order_model->getO2oErrandOrderInfo($conditions);
        if(!$o2o_errand_order_info){
            ds_json_encode(10001, '订单不存在');
        }

        if (!$order_model->editO2oErrandOrder(array('o2o_errand_order_state' => ORDER_STATE_SUCCESS, 'o2o_errand_order_finish_time' => TIMESTAMP), $conditions)) {
    ...
    public function evaluate() {
        ...
        $order_model = model('o2o_errand_order');
        $conditions = array('o2o_errand_order_id' => $o2o_errand_order_id, 'member_id' => $this->member_info['member_id']);
        $o2o_errand_order_info = $order_model->getO2oErrandOrderInfo($conditions);
        if (!$o2o_errand_order_info) {
            ds_json_encode(10001, '订单不存在');
        }
