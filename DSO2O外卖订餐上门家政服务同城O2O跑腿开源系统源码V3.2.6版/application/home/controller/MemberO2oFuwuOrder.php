...
class MemberO2oFuwuOrder extends BaseMember {
    ...
    public function _initialize() {
        parent::_initialize();
        Lang::load(APP_PATH . 'home/lang/' . config('default_lang') . '/member_o2o_fuwu_order.lang.php');
    }
    ...
    public function index() {
        ...
        if ($o2o_fuwu_order_state !== null && $o2o_fuwu_order_state !== '') {
            $condition['o2o_fuwu_order_state'] = intval($o2o_fuwu_order_state);
        }
        ...
        if ($start_unixtime || $end_unixtime) {
            $condition['o2o_fuwu_order_add_time'] = array('between', array($start_unixtime ? $start_unixtime : 0, $end_unixtime ? ($end_unixtime + 86399) : TIMESTAMP));
        }
        $order_sn = input('param.order_sn');
        if ($order_sn) {
            $condition['o2o_fuwu_order_sn'] = $order_sn;
        }
        ...
        $this->assign('order_list', $o2o_fuwu_order_list);
        $this->assign('show_page', $o2o_fuwu_order_model->page_info->render());
    ...
    public function view() {
        ...
        $condition = array();
        $condition['member_id'] = $this->member_info['member_id'];
        $condition['o2o_fuwu_order_id'] = $o2o_fuwu_order_id;
        ...
        $this->assign('o2o_fuwu_order_info', $o2o_fuwu_order_info);
        $this->assign('o2o_fuwu_order_goods_spec_list', model('o2o_fuwu_order_goods_spec')->getO2oFuwuOrderGoodsSpecList(array('o2o_fuwu_order_id' => $o2o_fuwu_order_id)));
        $this->setMemberCurMenu('member_o2o_fuwu_order');
    ...
    public function buy() {
        ...
        $this->assign('week_array', array("周日", "周一", "周二", "周三", "周四", "周五", "周六"));
        $this->assign('if_predeposit', $this->member_info['available_predeposit'] > 0);
        $this->assign('if_rc_balance', $this->member_info['available_rc_balance'] > 0);
        //服务地址
        ...
        $this->assign('address_info', $address_receive_info);
        return $this->fetch($this->template_dir . 'buy');
    }
    ...
    public function add() {
        ...
        $o2o_fuwu_order_validate = validate('o2o_fuwu_order');
    ...
    public function evaluate() {
        ...
        $conditions = array('o2o_fuwu_order_id' => $o2o_fuwu_order_id, 'member_id' => $this->member_info['member_id']);
        $o2o_fuwu_order_info = $order_model->getO2oFuwuOrderInfo($conditions);
        if (!$o2o_fuwu_order_info) {
        ...
        if (request()->isPost()) {
            ...
            $o2o_fuwu_order_model->startTrans();
            try {
                $result = $o2o_fuwu_order_model->editO2oFuwuOrder(array('o2o_fuwu_order_if_evaluate' => 1, 'o2o_fuwu_order_evaluate_time' => TIMESTAMP, 'o2o_fuwu_order_evaluate_content' => $comment, 'o2o_fuwu_order_evaluate_score' => $score), $conditions);
                if (!$result) {
                    exception('评论保存失败');
                }
                //更新服务机构评分
                ...
                $o2o_fuwu_order_model->rollback();
                ds_json_encode(10001, $e->getMessage());
            }
            $o2o_fuwu_order_model->commit();

            ds_json_encode(10000, lang('ds_common_op_succ'));
        } else {
            $this->assign('o2o_fuwu_order_info',$o2o_fuwu_order_info);
            return $this->fetch($this->template_dir . 'evaluate');
        }
    }
    ...
    public function receive() {
        ...
        $order_model = model('o2o_fuwu_order');
        $conditions = array('member_id' => $this->member_info['member_id'], 'o2o_fuwu_order_state' => ORDER_STATE_SEND, 'o2o_fuwu_order_id' => $order_id);
        $o2o_fuwu_order_info = $order_model->getO2oFuwuOrderInfo($conditions);
        if (!$o2o_fuwu_order_info) {
            ds_json_encode(10001, '订单不存在');
        }

        $result = $order_model->receiveO2oFuwuOrder($conditions, 'buyer', $this->member_info['member_name']);
        if (!$result['code']) {
            ds_json_encode(10001, $result['msg']);
        }
