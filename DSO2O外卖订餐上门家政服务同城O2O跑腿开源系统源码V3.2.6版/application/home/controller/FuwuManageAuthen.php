...
class  FuwuManageAuthen extends BaseFuwu {
    ...
    public function _initialize() {
        parent::_initialize();
        Lang::load(APP_PATH . 'home/lang/'.config('default_lang').'/fuwu_manage_authen.lang.php');
    }
    
    
    function quality() {
        $this->assign('file_list',$this->get_info_file_list('quality'));
        /* 设置机构当前菜单 */
        $this->setFuwuCurMenu('fuwu_manage_authen_quality');
        /* 设置机构当前栏目 */
        $this->setFuwuCurItem('quality');
        return $this->fetch($this->template_dir.'quality');
    }
    
    function scene() {
        $this->assign('file_list',$this->get_info_file_list('scene'));
        /* 设置机构当前菜单 */
        $this->setFuwuCurMenu('fuwu_manage_authen_scene');
        /* 设置机构当前栏目 */
        $this->setFuwuCurItem('scene');
        return $this->fetch($this->template_dir.'scene');
    }
