...
function get_o2o_distributor_file($member_avatar,$type='avatar')
{
    if (empty($member_avatar)) {
        ...
        return UPLOAD_SITE_URL . '/' . ATTACH_O2O_DISTRIBUTOR . '/' . $member_avatar;
    }
}
function get_o2o_fuwu_file($id,$member_avatar,$type)
{
    if (empty($member_avatar)) {
        ...
        return UPLOAD_SITE_URL . '/' . ATTACH_O2O_FUWU_ORGANIZATION . '/' . $id . '/' . $member_avatar;
    }
}
function get_o2o_fuwu_class_logo($member_avatar) {
    if ($member_avatar) {
        return UPLOAD_SITE_URL . '/' . ATTACH_O2O_FUWU_CLASS . '/' .$member_avatar;
    }
}
function get_o2o_errand_class_logo($member_avatar) {
    if ($member_avatar) {
        return UPLOAD_SITE_URL . '/' . ATTACH_O2O_ERRAND_CLASS . '/' .$member_avatar;
    }
}
...
function o2o_send_order($order_list){
    $o2o_socket_url=config('o2o_socket_url');
    if(!$o2o_socket_url){
        return;
    }
    ...
    $o2o_script=<<<EOT
        ...
        script.onload = script.onreadystatechange = function() {
            if (!this.readyState || this.readyState === "loaded" || this.readyState === "complete" ) {
                ...
                socket = io("{$o2o_socket_url}", {
                    path: "/socket.io",
                });
                socket.on("connect", function () {
                    for(var i in order_list){
                        socket.emit("addOrder", order_list[i]);
                    }
                });
        ...
        script.src= "{$o2o_socket_url}/socket.io/socket.io.js";
