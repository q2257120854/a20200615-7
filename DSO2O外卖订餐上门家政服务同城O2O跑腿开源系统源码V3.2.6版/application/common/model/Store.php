...
class  Store extends Model {
    ...
    private function getGoodsCountJq($store_array) {
        $order_count_array = db('order')->field('store_id,count(*) as order_count')->where(array('store_id' => array('in', implode(',', array_keys($store_array))),'order_state'=>array('NEQ',"0"), 'add_time' => array('gt', TIMESTAMP - 3600 * 24 * 90)))->group('store_id')->select();
    ...
    public function isO2oSupport($store_info){
        if($store_info['store_o2o_distribute_type']==1 || ($store_info['store_o2o_distribute_type']==0 && config('o2o_open') && ((!$store_info['is_platform_store'] && $store_info['store_o2o_support']==1) || $store_info['is_platform_store']))){
            return true;
        }else{
            return false;
        }
