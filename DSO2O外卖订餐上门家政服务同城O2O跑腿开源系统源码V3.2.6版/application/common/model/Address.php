...
class  Address extends Model {
    ...
    public function addAddress($data) {
        ...
        if($data['address_is_default']==1){
            ...
            if(!isset($data['address_o2o_errand_type'])){
                ...
                $condition['address_o2o_errand_type']=$data['address_o2o_errand_type'];
            }
            db('address')->where($condition)->update(array('address_is_default'=>0));
        }
    ...
    public function editAddress($update, $condition) {
        ...
        if($update['address_is_default']==1 && $condition['member_id']>0){
            if(!isset($update['address_o2o_errand_type'])){
                $condition['address_o2o_errand_type']=0;
            }else{
                $condition['address_o2o_errand_type']=$update['address_o2o_errand_type'];
            }
            db('address')->where($condition)->update(array('address_is_default'=>0));
        }
