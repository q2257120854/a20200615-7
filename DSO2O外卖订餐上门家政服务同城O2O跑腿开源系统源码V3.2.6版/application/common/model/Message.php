...
class Message extends Model
{
    ...
    public function getMessageList($condition, $pagesize = '')
    {
        ...
        $message_list= db('message')->where($where)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
    ...
    public function getStoreMessageList($condition, $pagesize = '')
    {
        ...
        $message_list=db('message')->join('__STORE__','message.from_member_id = store.member_id','LEFT')->field($field)->where($where)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
        $this->page_info=$message_list;
        $message=$message_list->items();
        return $message;
    }
    ...
    public function delCommonMessage($condition, $drop_type)
    {
        ...
        $message_list = db('message')->alias('message')->where($where)->field($field)->select();
        unset($where);
    ...
    private function getCondition($condition_array,$join=true)
    {
        ...
        if($join) {
            ...
            if (isset($condition_array['from_to_member_id']) && $condition_array['from_to_member_id'] != '') {
                $condition_sql .= " and (from_member_id='{$condition_array['from_to_member_id']}' or to_member_id ='all' or (message_ismore=0 and to_member_id='{$condition_array['from_to_member_id']}') or (message_ismore=1 and to_member_id like '%,{$condition_array['from_to_member_id']},%'))";
            }
            //未删除
            if (isset($condition_array['no_del_member_id']) && $condition_array['no_del_member_id'] != '') {
