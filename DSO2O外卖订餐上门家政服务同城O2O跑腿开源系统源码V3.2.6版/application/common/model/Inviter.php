...
class  Inviter extends Model {
     ...
     * @return array
     */
    public function getInviterList($condition = array(), $pagesize = '', $limit = '', $fields = '*') {
        if($pagesize) {
            $res=db('inviter')->alias('i')->join('__MEMBER__ m', 'i.inviter_id=m.member_id')->field($fields)->where($condition)->order('inviter_applytime desc')->paginate($pagesize,false,['query' => request()->param()]);
            $this->page_info=$res;
            $result=$res->items();
        }else{
    ...
    public function qrcode_logo($member_info){
        ...
        if (!file_exists($qrcode_path)) {
            ...
            \QRcode::png(config('h5_site_url') . '/home/memberregister?inviter_id=' . $member_info['member_id'], $qrcode_path);
        }
