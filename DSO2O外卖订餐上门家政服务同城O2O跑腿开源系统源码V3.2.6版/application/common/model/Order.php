...
class  Order extends Model {
    ...
    public function getOrderpayInfo($condition = array()) {
        return db('orderpay')->where($condition)->find();
    }
    ...
    public function getOrderpayList($condition, $field = '*', $order = '', $key = '') {
        $pay_list = db('orderpay')->field($field)->where($condition)->order($order)->select();
        if ($key) {
    ...
    public function getOrderList($condition, $pagesize = '', $field = '*', $order = 'order_id desc', $limit = '', $extend = array()) {
        ...
        if ($pagesize) {
            $list_paginate = db('order')->field($field)->where($condition)->order($order)->paginate($pagesize, false, ['query' => request()->param()]);
     ...
     * @return array
     */
    public function getOrdergoodsList($condition = array(), $fields = '*', $limit = null, $pagesize = null, $order = 'rec_id desc', $group = null, $key = null) {
        if ($pagesize) {
            $res = db('ordergoods')->field($fields)->where($condition)->order($order)->group($group)->paginate($pagesize, false, ['query' => request()->param()]);
            $this->page_info = $res;
    ...
    public function getOrderOperateState($operate, $order_info) {
        ...
        switch ($operate) {
            ...
            //派单
            case 'deliver':
                $state = !$order_info['lock_state'] && $order_info['order_state'] == ORDER_STATE_RECEIPT;
                break;
            //取货
            case 'pickup':
    ...
    public function getOrderAndOrderGoodsList($condition, $field = '*', $pagesize = 0, $order = 'rec_id desc') {
        if ($pagesize) {
            $list = db('ordergoods')->alias('order_goods')->where($condition)->field($field)->join('__ORDER__ order', 'order_goods.order_id=order.order_id', 'LEFT')->paginate($pagesize, false, ['query' => request()->param()]);
            $this->page_info = $list;
            return $list->items();
        } else {
    ...
    public function autoOrderDeliver($store_info, $order_info) {
        ...
        if ($store_info['store_o2o_auto_deliver'] == 1) {//如果开启了自动派单
            if ($order_info['o2o_order_distributor_type'] == 1) {
                ...
                $o2o_distributor_info = db('o2o_distributor')->alias('o2o_distributor')->join('__ORDER__ order_alias', 'order_alias.o2o_distributor_id=o2o_distributor.o2o_distributor_id', 'LEFT')->where(array('o2o_distributor.store_id' => $store_info['store_id'], 'o2o_distributor.o2o_distributor_state' => 1, 'o2o_distributor.o2o_distributor_receipt' => 1, 'o2o_distributor.o2o_distributor_auto_receipt' => 1))->group('o2o_distributor.o2o_distributor_id')->having('CASE WHEN o2o_distributor.o2o_distributor_receipt_limit>0 THEN COUNT(order_alias.order_state IN (' . ORDER_STATE_DELIVER . ',' . ORDER_STATE_SEND . ') OR NULL) < o2o_distributor.o2o_distributor_receipt_limit ELSE 1=1 END')->orderRaw('COUNT(order_alias.order_state IN (' . ORDER_STATE_DELIVER . ',' . ORDER_STATE_SEND . ') OR NULL) ASC')->field('o2o_distributor.o2o_distributor_id,o2o_distributor.o2o_distributor_name,o2o_distributor.o2o_distributor_realname,o2o_distributor.o2o_distributor_phone')->find();
            } else {//如果是平台配送员，则匹配配送地区在店铺所在地区的配送员
                $o2o_distributor_info = db('o2o_distributor')->alias('o2o_distributor')->join('__ORDER__ order_alias', 'order_alias.o2o_distributor_id=o2o_distributor.o2o_distributor_id', 'LEFT')->where(array('o2o_distributor.store_id' => 0, 'o2o_distributor.o2o_distributor_region_id' => $store_info['region_id'], 'o2o_distributor.o2o_distributor_state' => 1, 'o2o_distributor.o2o_distributor_receipt' => 1, 'o2o_distributor.o2o_distributor_auto_receipt' => 1))->group('o2o_distributor.o2o_distributor_id')->having('CASE WHEN o2o_distributor.o2o_distributor_receipt_limit>0 THEN COUNT(order_alias.order_state IN (' . ORDER_STATE_DELIVER . ',' . ORDER_STATE_SEND . ') OR NULL) < o2o_distributor.o2o_distributor_receipt_limit ELSE 1=1 END')->orderRaw('COUNT(order_alias.order_state IN (' . ORDER_STATE_DELIVER . ',' . ORDER_STATE_SEND . ') OR NULL) ASC')->field('o2o_distributor.o2o_distributor_id,o2o_distributor.o2o_distributor_name,o2o_distributor.o2o_distributor_realname,o2o_distributor.o2o_distributor_phone')->find();
            }
    ...
    public function formatO2oOrder($store_info, $order_info) {
        ...
        //商品数
        $order_info['goods_num'] = db('ordergoods')->where(array('order_id' => $order_info['order_id']))->sum('goods_num');
        //收货地址
        ...
        $order_info['buyer_address'] = $ordercommon_info['reciver_info']['address'];
        //店铺地址
