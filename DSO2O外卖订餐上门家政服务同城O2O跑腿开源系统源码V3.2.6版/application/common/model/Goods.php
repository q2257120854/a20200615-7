...
class  Goods extends Model {
    ...
    public function getGoodsList($condition, $field = '*', $group = '', $order = '', $limit = 0, $pagesize = 0, $lock = false, $count = 0) {
        ...
        if ($pagesize) {
            $result = db('goods')->field($field)->where($condition)->group($group)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
    ...
    public function getGoodsSpecListByCommonId($common_id) {
        ...
        if (empty($spec_list)) {
            $spec_array = $this->getGoodsList(array('goods_commonid' => $common_id), 'goods_spec,goods_promotion_price,goods_id,store_id,goods_image,color_id');
