...
class  Goodsclasstag extends Model {
    ...
    public function getGoodsclasstagList($condition = array(), $pagesize = '', $field = '*') {
        if ($pagesize) {
            $result = db('goodsclasstag')->field($field)->where($condition)->paginate($pagesize,false,['query' => request()->param()]);
    ...
    public function addGoodsclasstag($param) {
        ...
        return Db::execute("insert into `" . config('database.prefix')."goodsclasstag` (`gc_id_1`,`gc_id_2`,`gc_id_3`,`gctag_name`,`gctag_value`,`gc_id`,`type_id`) values " . $condition_str);
    }
    ...
    public function clearGoodsclasstag() {
        return Db::execute("TRUNCATE TABLE `" . config('database.prefix') . "goodsclasstag`");
    }
