 ...
 * 数据层模型
 */
class  O2oFuwuClass extends Model {
    public $page_info;
     ...
     * @return array
     */
    public function getO2oFuwuClassList($condition = array(), $fields = '*', $pagesize = null, $order = 'o2o_fuwu_class_id asc', $limit = null) {
        if($pagesize){
            $result = db('o2o_fuwu_class')->where($condition)->field($fields)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
            $this->page_info = $result;
            return $result->items();
        }else{
            return db('o2o_fuwu_class')->where($condition)->field($fields)->order($order)->limit($limit)->select();
        }
    ...
    public function getO2oFuwuClassInfo($condition = array(), $fields = '*') {
        return db('o2o_fuwu_class')->where($condition)->field($fields)->find();
    }
    ...
    public function addO2oFuwuClass($data) {
        return db('o2o_fuwu_class')->insertGetId($data);
    }
    ...
    public function editO2oFuwuClass($data, $condition = array()) {
        return db('o2o_fuwu_class')->where($condition)->update($data);
    }
    ...
    public function delO2oFuwuClass($condition,$o2o_fuwu_class=array()) {
        ...
        return db('o2o_fuwu_class')->where($condition)->delete();
    }
