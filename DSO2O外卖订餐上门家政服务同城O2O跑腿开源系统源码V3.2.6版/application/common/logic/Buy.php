...
class  Buy extends Model {
        ...
        foreach ($store_cart_list as $store_id => $store_cart) {
            if ($store_model->isO2oSupport($this->_store_list[$store_id])) {
                if ($this->_store_list[$store_id]['store_o2o_distribute_type'] == 1) {
                    $result['store_o2o_fee_list'][$store_id] = floatval($this->_store_list[$store_id]['store_o2o_fee']);
                } else {
                    $result['store_o2o_fee_list'][$store_id] = floatval(config('o2o_fee'));
                }
            } else {
    ...
    private function _createOrderStep4() {
        ...
        $store_list = $this->_store_list;
        $res = $this->_logic_buy_1->calcDistance($input_address_info['address_id'], $store_list);
        if (!$res['state']) {
            exception($res['msg']);
        }
