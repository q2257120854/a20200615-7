...
class  Buy_1 extends Model {
    ...
    public function calcDistance($address_id, $store_list) {

        if (empty($address_id) || empty($store_list)) {
            $res = array('state' => false, 'msg' => lang('param_error'));
        } else {
            $address_info = db('address')->where(array('address_id' => $address_id, 'member_id' => session('member_id')))->field('address_longitude,address_latitude')->find();
            if (!$address_info) {
                ...
                foreach ($store_list as $store_info) {
                    $dist = getDistance($store_info['store_latitude'], $store_info['store_longitude'], $address_info['address_latitude'], $address_info['address_longitude']);
                    if (model('store')->isO2oSupport($store_info)) {
                        //如果是店铺配送
                        $dist2 = ($store_info['store_o2o_distribute_type'] == 1) ? $store_info['store_o2o_distance'] : config('o2o_distance');
                        if ($dist > $dist2) {//是否超出配送距离
                            $res = array('state' => false, 'msg' => sprintf('您的地址%sm超出%s的配送距离%sm', $dist, $store_info['store_name'], $dist2));
                            break;
                        }
                        if ($store_info['store_o2o_distribute_type'] == 1) {
                            $data[$store_info['store_id']] = array('distributor_type' => 1, 'o2o_fee' => $store_info['store_o2o_fee']);
                        } else {
                            $data[$store_info['store_id']] = array('distributor_type' => 0, 'o2o_fee' => config('o2o_fee'));
                        }
