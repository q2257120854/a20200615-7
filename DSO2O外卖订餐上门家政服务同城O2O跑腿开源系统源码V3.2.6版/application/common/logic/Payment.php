...
class  Payment extends Model
{
    ...
    public function getO2oErrandOrderInfo($pay_sn, $member_id = null)
    {
        ...
        $order_info = $o2o_errand_order_model->getO2oErrandOrderInfo(array('member_id'=>$member_id,'o2o_errand_order_sn'=>$pay_sn));
        if (!$order_info) {
            return ds_callback(false, '该订单不存在');
        }
        ...
        $order_info['api_pay_amount'] = $order_info['o2o_errand_order_amount'];
        return ds_callback(true, '', $order_info);
    }
    /**
     * 取得服务订单所需支付金额等信息
    ...
    public function getO2oFuwuOrderInfo($pay_sn, $member_id = null)
    {
        ...
        $order_info = $o2o_fuwu_order_model->getO2oFuwuOrderInfo(array('member_id'=>$member_id,'o2o_fuwu_order_sn'=>$pay_sn));
        if (!$order_info) {
            return ds_callback(false, '该订单不存在');
        }
    ...
    public function updateOrder($out_trade_no,$trade_no,$order_type,$payment_code,$payment_name=''){
        ...
        if ($order_type == 'real_order') {
            ...
            $result = model('o2o_errand_order')->payO2oErrandOrder($out_trade_no, $payment_code, $trade_no,'system');
            ...
            $result = model('o2o_fuwu_order')->payO2oFuwuOrder($out_trade_no, $payment_code, $payment_name, $trade_no,'system');
        }
