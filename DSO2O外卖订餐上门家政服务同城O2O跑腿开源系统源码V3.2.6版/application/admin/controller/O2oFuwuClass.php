...
class  O2oFuwuClass extends AdminControl {
    ...
    public function _initialize() {
        parent::_initialize();
        Lang::load(APP_PATH . 'admin/lang/'.config('default_lang').'/o2o_fuwu_class.lang.php');
    }
    ...
    public function edit() {
        ...
        if (!request()->isPost()) {
            $this->get_common_data();
            $this->assign('o2o_fuwu_class', $o2o_fuwu_class);
            return $this->fetch('form');
        } else {
            $data = $this->post_data();
            ...
            $condition=array();
            $condition['o2o_fuwu_class_id']=$o2o_fuwu_class_id;
    ...
    public function post_data(){
        ...
        $o2o_fuwu_class_validate=validate('o2o_fuwu_class');
        ...
        if (!empty($_FILES['_pic']['name'])) {
            $upload_file = BASE_UPLOAD_PATH . '/' . ATTACH_O2O_FUWU_CLASS;
            $file = request()->file('_pic');
            $result = $file->rule('uniqid')->validate(['ext' => ALLOW_IMG_EXT])->move($upload_file);
            if ($result) {
                $data['o2o_fuwu_class_logo'] = $result->getFilename();
            }
        }
        return $data;
    }
