...
class  Storeclass extends AdminControl {
        ...
        if (!request()->isPost()) {
            ...
            if ($result) {
                ...
                if (!empty($_FILES['pic']['name'])) {//上传图片
                    $upload_file = BASE_UPLOAD_PATH . DS . ATTACH_STORECLASS;
                    if (!empty($_FILES['pic']['name'])) {//上传图片
                        $file = request()->file('pic');
                        $file->validate(['ext' => ALLOW_IMG_EXT])->move($upload_file, 'storeclass-pic-' . $result . '.png');
                    }
                }
    ...
    public function store_class_edit() {
        ...
        if (!request()->isPost()) {
            $storeclass = $storeclass_model->getStoreclassInfo(array('storeclass_id' => $storeclass_id));
            if (empty($storeclass)) {
            ...
            $result = $storeclass_model->editStoreclass($update_array, array('storeclass_id' => $storeclass_id));
            if (!empty($_FILES['pic']['name'])) {//上传图片
                $upload_file = BASE_UPLOAD_PATH . DS . ATTACH_STORECLASS;
                if (!empty($_FILES['pic']['name'])) {//上传图片
                    $file = request()->file('pic');
                    $file->validate(['ext' => ALLOW_IMG_EXT])->move($upload_file, 'storeclass-pic-' . $storeclass_id . '.png');
                }
            }
