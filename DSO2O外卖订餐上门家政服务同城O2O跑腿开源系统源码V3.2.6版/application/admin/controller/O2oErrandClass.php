...
class  O2oErrandClass extends AdminControl {
    ...
    public function _initialize() {
        parent::_initialize();
        Lang::load(APP_PATH . 'admin/lang/'.config('default_lang').'/o2o_errand_class.lang.php');
    }
    ...
    public function index() {
        ...
        $this->assign('o2o_errand_class_list', $o2o_errand_class_list);
        $this->assign('show_page', $o2o_errand_class_model->page_info->render());
    ...
    public function add() {
        if (!(request()->isPost())) {
            ...
            $data = $this->post_data();
            $o2o_errand_class_validate = validate('o2o_errand_class');
    ...
    public function edit() {
        ...
        if (!request()->isPost()) {
            ...
            $this->assign('o2o_errand_class', $o2o_errand_class);
            ...
            $o2o_errand_class_model= model('o2o_errand_class');
            $condition=array();
            $condition['o2o_errand_class_id']=$o2o_errand_class_id;
    ...
    public function post_data() {
        ...
        if (!empty($_FILES['_pic']['name'])) {
            $upload_file = BASE_UPLOAD_PATH . '/' . ATTACH_O2O_ERRAND_CLASS;
            $file = request()->file('_pic');
            $result = $file->rule('uniqid')->validate(['ext' => ALLOW_IMG_EXT])->move($upload_file);
            if ($result) {
                $data['o2o_errand_class_logo'] = $result->getFilename();
            }
        }
        return $data;
    }
