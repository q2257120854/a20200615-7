...
class  Voucher extends Model
{
    ...
    public function getCanChangeTemplateInfo($vid, $member_id, $store_id = 0,$no_point=false)
    {
        ...
        if ($store_id > 0 && $store_id == $template_info['vouchertemplate_store_id']) {
            return array('state' => false, 'msg' => '不可以兑换自己机构的代金券');
        }
        ...
        if (!empty($voucher_list)) {
            ...
            //买家最多只能拥有同一个机构尚未消费抵用的机构代金券最大数量的验证
    ...
    public function getMemberVoucherList($member_id, $voucher_state, $pagesize = null)
    {
       ...
       $list = db('voucher')->alias('v')->join('__STORE__ s','s.store_id=v.voucher_store_id')->join('__VOUCHERTEMPLATE__ t','v.vouchertemplate_id=t.vouchertemplate_id')->where($where)->order('voucher_id desc')->paginate($pagesize,false,['query' => request()->param()]);
        $this->page_info=$list;
        $list=$list->items();
    ...
    public function getVoucherquotaList($condition,$pagesize,$order){
        if($pagesize){
            $voucherquota_list = db('voucherquota')->where($condition)->order($order)->paginate($pagesize, false, ['query' => request()->param()]);
