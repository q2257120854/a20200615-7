...
class  Favorites extends Model
{
    ...
    {
        if ($pagesize) {
        $res= db('favorites')->where($condition)->field($field)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
        $this->page_info=$res;
        return $res->items();
        } else {
    ...
    public function addFavorites($data)
    {
        ...
        if ($data['fav_type'] == 'goods') {
            ...
            $data['favlog_price'] = $goods['goods_price'];//商品收藏时价格
            $data['favlog_msg'] = $goods['goods_price'];//收藏备注，默认为收藏时价格，可修改
    ...
    public function getStoreFavoritesCountByMemberId($id)
    {
        ...
        return db('favorites')->where($where)->count();
    }
    /**
     * 获取商品收藏数
     * @access public
