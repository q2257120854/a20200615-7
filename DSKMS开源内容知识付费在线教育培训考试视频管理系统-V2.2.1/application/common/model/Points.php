...
class  Points extends Model {
     ...
     * @return type
     */
    public function getPointslogList($condition, $pagesize = '', $field = '*',$limit='') {
        $order = isset($condition['order']) ? $condition['order'] : 'pl_id desc';
        if ($pagesize) {
            $result = db('pointslog')->where($condition)->field($field)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
