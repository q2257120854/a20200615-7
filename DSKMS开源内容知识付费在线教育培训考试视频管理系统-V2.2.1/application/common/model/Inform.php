...
class  Inform extends Model {
    ...
    public function getInformList($condition = '', $pagesize = '',$order = 'inform_id desc') {
        $res = db('inform')->alias('inform')->join('__INFORMSUBJECT__ inform_subject', 'inform.informsubject_id = inform_subject.informsubject_id', 'LEFT')->where($condition)->order($order)->paginate($pagesize, false, ['query' => request()->param()]);
        $this->page_info = $res;
        return $res->items();
    }
