...
class  Pointorder extends Model {
    ...
    public function getPointorderList($where, $field = '*', $pagesize = 0, $limit = 0, $order = '') {
        ...
        if($pagesize){
            $order_list = db('pointsorder')->field($field)->where($where)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
    ...
    public function getPointorderAndGoodsList($where, $field = '*',  $order = '',$pagesize='') {
        if($pagesize){
            $order_list = db('pointsordergoods')->alias('pointsordergoods')->field($field)->join('__POINTSORDER__ pointsorder','pointsordergoods.pointog_orderid=pointsorder.point_orderid')->where($where)->order($order)->paginate($pagesize,false,['query' => request()->param()]);
            $this->page_info = $order_list;
            $order_list = $order_list->items();
            return $order_list;
        }else{
