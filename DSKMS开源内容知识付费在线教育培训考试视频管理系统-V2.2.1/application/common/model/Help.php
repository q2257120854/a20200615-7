...
class  Help extends Model {
    ...
    public function getHelptypeList($condition = array(), $pagesize = '', $fields = '*') {
        $result = db('helptype')->field($fields)->where($condition)->order('helptype_sort asc,helptype_id desc')->paginate($pagesize,false,['query' => request()->param()]);
     ...
     * @return array
     */
    public function getHelpList($condition = array(), $pagesize = '', $limit = '', $fields = '*') {
        if($pagesize) {
            $res=db('help')->field($fields)->where($condition)->order('help_sort asc,help_id desc')->paginate($pagesize,false,['query' => request()->param()]);
            $this->page_info=$res;
            $result=$res->items();
        }else{
    ...
    public function getStoreHelptypeList($condition = array(), $pagesize = '', $limit = 0) {
        $condition['page_show'] = '1'; //页面类型:1为机构,2为会员
        $res = db('helptype')->where($condition)->order('helptype_sort asc,helptype_id desc')->paginate($pagesize,false,['query' => request()->param()]);
        $this->page_info=$res;
        $result=$res->items();
