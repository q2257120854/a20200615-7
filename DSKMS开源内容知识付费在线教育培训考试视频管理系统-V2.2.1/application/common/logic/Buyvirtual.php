...
class  Buyvirtual extends Model
{
    ...
    public function getBuyStepData($goods_id, $quantity, $member_id) {
        ...
        $return['store_voucher_list']=array();
            if (config('voucher_allow')){
              ...
              $condition = array();
              $condition['voucher_store_id'] = $goods_info['store_id'];
    ...
    public function addOrderInviter($order = array()) {
        ...
        $goods = $order;
        //查询商品的分销信息
        $goods_common_info = db('goods')->where('goods_id=' . $goods['goods_id'])->field('goods_id,inviter_open,inviter_ratio_1,inviter_ratio_2,inviter_ratio_3')->find();
