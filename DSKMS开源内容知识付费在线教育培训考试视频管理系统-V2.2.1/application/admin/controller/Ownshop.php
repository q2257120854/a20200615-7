...
class  Ownshop extends AdminControl {
    ...
    public function add() {
        if (!request()->isPost()) {
            ...
            $this->log("新增自营机构: {$saveArray['store_name']}");
    ...
    public function del() {
        ...
        if (!$storeArray['is_platform_store']) {
            $this->error('不能在此删除非自营机构');
        }
        $condition = array(
            'store_id' => $store_id,
        );
        if (model('goods')->getGoodsCount($condition) > 0)
            $this->error('已经发布商品的自营机构不能被删除');

        // 完全删除机构
        $store_model->delStoreEntirely($condition);
        // 删除自营店id缓存
        model('store')->dropCachedOwnShopIds();
        $this->log("删除自营机构: {$storeArray['store_name']}");
    ...
    public function edit() {
        ...
        if (!request()->isPost()) {
            ...
            $this->log("编辑自营机构: {$saveArray['store_name']}");
            dsLayerOpenSuccess(lang('ds_common_op_succ'),url('Ownshop/index'));
        }
    }
    ...
    public function bind_class_add() {
        ...
        if ($result) {
            ...
            $this->log('增加自营机构经营类目，类目编号:' . $result . ',机构编号:' . $store_id);
    ...
    public function bind_class_del() {
        ...
        foreach ($bid_array as $key => $bid) {
            ...
            // 删除自营店id缓存
            model('store')->dropCachedOwnShopIds();
            $this->log('删除自营机构经营类目，类目编号:' . $bid . ',机构编号:' . $store_bind_class_info['store_id']);
        }
    ...
    public function ajax() {
        ...
        switch (input('param.branch')) {
            ...
            case 'store_sort':
                ...
                if($result){
                    $this->log(lang('ds_edit').'自营机构' . '[' . $id . ']', 1);
                }
                echo 'true';
                exit;
                break;
    ...
    protected function getAdminItemList() {
        $menu_array = array(
            array(
                ...
                'name' => 'add',
                'text' => '新增',
                'url' => "javascript:dsLayerOpen('".url('Ownshop/add')."','新增自营机构')"
            )
        );
