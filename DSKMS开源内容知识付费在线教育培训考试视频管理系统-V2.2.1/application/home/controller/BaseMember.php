...
class BaseMember extends BaseHome {
    ...
    public function _initialize() {
        ...
        /*非登录状态*/
        if (!session('is_login')) {
            //控制器不为Cart 以及 控制器为Cart中的非指定方法
            if (!in_array(request()->controller(), array('Cart')) ||  (in_array(request()->controller(), array('Cart')) && !in_array(request()->action(), array('ajax_load', 'add', 'del'))) ) {
                $ref_url = request_uri();
...
//                    array('name' => 'member_invoice', 'text' => lang('ds_member_invoice'), 'url' => url('Memberinvoice/index'),),
                    ...
                    array('name' => 'member_exampaper', 'text' => lang('ds_memberexampaper_log'), 'url' => url('Memberexampaper/log'),),
                )
            ),
            'assets' =>
            array(
            ...
            array(
                ...
                'submenu' => array(
                    array('name' => 'member_refund', 'text' => lang('ds_refund_and_return'), 'url' => url('Membervrrefund/index'),),
