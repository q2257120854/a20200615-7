...
class  Sellerinviter extends BaseSeller {
    ...
    public function _initialize() {
        ...
        if (intval(config('inviter_open')) !== 1) {
    ...
    public function goods_list() {
        ...
        $goods_list = $goods_model->getGoodsList($condition, '*', 10);
    ...
    public function goods_add() {
        $goods_model = model('goods');
        if (!request()->isPost()) {
            $this->assign('inviter_return', config('inviter_return'));
            ...
            $goods_info = $goods_model->getGoodsInfo('goods_id=' . $inviter_goods_id . ' AND store_id=' . session('store_id'));
            if (!$goods_info) {
    ...
    public function goods_edit() {
        ...
        if (!request()->isPost()) {
            ...
            $goods_info = $goods_model->getGoodsInfo('goods_id=' . $goods_id . ' AND inviter_open=1 AND store_id=' . session('store_id'));
            if (!$goods_info) {
            ...
            $goods_info = $goods_model->getGoodsInfo('goods_id=' . $inviter_goods_id . ' AND inviter_open=1 AND store_id=' . session('store_id'));
            if (!$goods_info) {
                ds_json_encode(10001, lang('sellerinviter_goods_empty'));
            }
            if ($inviter_ratio_1 > config('inviter_ratio_1')) {
    ...
    public function search_goods() {
        ...
        $condition['store_id'] = session('store_id');
        $condition['goods_name'] = array('like', '%' . input('param.goods_name') . '%');
        $goods_list = $goods_model->getGoodsList($condition, '*', 8);
        $this->assign('goods_list', $goods_list);
        $this->assign('show_page', $goods_model->page_info->render());
