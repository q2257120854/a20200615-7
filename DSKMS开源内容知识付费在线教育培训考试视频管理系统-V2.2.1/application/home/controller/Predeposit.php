...
class  Predeposit extends BaseMember {
    ...
    public function pd_cash_add() {
        if (request()->isPost()) {
            ...
            //是否超过提现周期
            $last_withdraw = $predeposit_model->getPdcashInfo(array('pdc_member_id' => $this->member_info['member_id'], 'pdc_payment_state' => array('in', [0, 1]), 'pdc_addtime' => array('>', TIMESTAMP - intval(config('member_withdraw_cycle')) * 86400)));
            if ($last_withdraw) {
                ds_json_encode(10001, lang('predeposit_last_withdraw_time_error') . date('Y-m-d', $last_withdraw['member_withdraw_cycle']));
            }
            //是否不小于最低提现金额
            if ($pdc_amount < floatval(config('member_withdraw_min'))) {
                ds_json_encode(10001, lang('predeposit_withdraw_min') . config('member_withdraw_min') . lang('currency_zh'));
            }
            //是否不超过最高提现金额
            if ($pdc_amount > floatval(config('member_withdraw_max'))) {
                ds_json_encode(10001, lang('predeposit_withdraw_max') . config('store_withdraw_max') . lang('currency_zh'));
            }
            try {
