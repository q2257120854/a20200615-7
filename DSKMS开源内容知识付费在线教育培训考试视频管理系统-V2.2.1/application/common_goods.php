...
/**
 * 取得商品缩略图的完整URL路径，接收图片名称与机构ID
 *
 * @param string $file 图片名称
 * @param string $type 缩略图尺寸类型，值为240,480,1280
 * @param mixed $store_id 机构ID 如果传入，则返回图片完整URL,如果为假，返回系统默认图
 * @return string
 */
 ...
 * @return string
 */
function goods_bgthumb($file, $type = '', $store_id = false)
{
    $type_array = explode(',_', ltrim(GOODS_IMAGES_EXT, '_'));
    if($type=='default'){
        $type='';
    ...
    if (empty($file)) {
        return UPLOAD_SITE_URL . '/' . ATTACH_COMMON . '/default_goods_bg_image.jpg' ;
    }
    $fname = basename($file);
    // 取机构ID
    $upload_type = explode('_', $fname);
