...
<section class="rt_wrap content mCustomScrollbar">
 <div class="rt_content">
      <div class="page_title">
       <h2 class="fl">会员列表</h2>
       <a href="<?php echo url('users/add'); ?>" class="fr top_rt_btn add_icon">添加会员</a>
      </div>
      <section class="mtb">
...
<!--    <select class="select" name="designer">
	   ...
	   <a href="<?php echo url('users/lists'); ?>?status=1" class="home"/>冻结</a>
	   <a href="<?php echo url('users/lists'); ?>" class="all"/>全部</a>  &nbsp;&nbsp;  <span style=""/>会员总人数：<?php echo $userTotal; ?> 人</span> <span style=""/>最新注册：<?php echo $newNum; ?> 人</span> 
	   </form>
      </section>
      <table class="table">
       <tr>
       ...
       <tr class="id_<?php echo $vo['id']; ?>">
		...
		<td class="center">
			￥ <?php echo $vo['money_wallet']; ?>
		</td>
        <td class="center">
			￥ <?php echo $vo['ore_wallet']; ?>
		</td>
        <td class="center">
			￥ <?php echo $vo['forepart_money']; ?> 
		</td>
        <td class="center">
			￥ <?php echo $vo['release_wallet']; ?> 
		</td>
        <td class="center">
			<?php echo $vo['team_num']; ?> &nbsp;人
		</td>
		<td class="center"><?php if($vo['status'] == 1): ?><a title="正常" class="link_icon">&#89;</a><?php else: ?><a title="冻结" class="link_icon" style="color:red;">&#88;</a><?php endif; ?></td>
		<td class="center"><?php echo date("Y-m-d",$vo['addtime']); ?></td>
        <td class="center">
         <a href="<?php echo url('users/edit'); ?>?id=<?php echo $vo['id']; ?>" title="编辑" class="link_icon">&#101;</a>
...
$(document).ready(function() {
	...
	$('#delid').click(function(event){
		...
		if (confirm("确定要删除吗？")) {
			$.ajax({
				url:"<?php echo url('users/delmost'); ?>",
				dataType:"json",
				type:'POST',
				cache:false,
