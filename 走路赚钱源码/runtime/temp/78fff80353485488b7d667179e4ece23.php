 ...
 <div class="rt_content">
      ...
      <table class="table">
       ...
       <tr class="id_<?php echo $vo['id']; ?>">
        ...
        <td class="center"><?php echo $vo['description']; ?></td>
        <td class="center"><?php echo $vo['area']; ?></td>      
        <td class="center"><?php echo $vo['loginip']; ?></td>
...
$(document).ready(function() {
  ...
  //删除
	$('.del').click(function(event){
		...
		$.ajax({
			url:"<?php echo url('system/log_del'); ?>",
			dataType:"json",
			type:'POST',
			cache:false,
			data:{op:'logdel',id:id},
			success: function(data) {
				console.log(data.s);
				if (data.s=='ok') {				
					myAlert('删除成功');
	...
	$('#delid').click(function(event){
		...
		$.ajax({
			url:"<?php echo url('system/log_delmost'); ?>",
			dataType:"json",
			type:'POST',
			cache:false,
			data:{op:'logDelAll',delid:id_str},
			success: function(data) {
				console.log(data.s);
				if (data.s=='ok') {				
					myAlert('删除成功');
