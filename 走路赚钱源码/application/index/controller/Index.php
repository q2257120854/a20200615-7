...
/* 		//卖出订单
		...
		$id=$_SESSION['user_id'];
		$cmm_users=Db::table('cmm_users')->where('id',$id)->find();
		$this->assign('eth_purse',$cmm_users['eth_purse']);//以太防地址
...
/* 		$this->assign('selllog',$selllog);//卖出订单
	...
	public function zichan(){
		$id=$_SESSION['user_id'];
		$cmm_users=Db::table('cmm_users')->where('id',$id)->find();
		$this->assign('usdt',$cmm_users['usdt']);//购机币
		$this->assign('money_wallet',$cmm_users['money_wallet']);//乐豆种子帐户
		$this->assign('eth_purse',$cmm_users['eth_purse']);//以太防地址
		return $this->fetch('./zichan.html');
	} 
	//充值
	public function chongzhi(){
		$id=$_SESSION['user_id'];
		//$cmm_users=Db::table('cmm_users')->where('id',$id)->find();
		$this->assign('userid',$id);//购机币
		return $this->fetch('./chongzhi.html');
	} 
	//提现
	...
	public function getimage(){
		...
		if($file){
			$info = $file->move(ROOT_PATH . 'public/uploads/tixian');
			if($info){
				// 成功上传后 获取上传信息
				// 输出 20160820/42a79759f284b767dfcb2a0197904287.jpg
				echo '/uploads/tixian/'.$info->getSaveName();
			}else{
				// 上传失败获取错误信息
				echo $file->getError();
			}
	...
	//自助购买
	public function zizhubuy(){
		
		$id=$_SESSION['user_id'];
		$cmm_users=Db::table('cmm_users')->where('id',$id)->find();
		$this->assign('userid',$id);
		$this->assign('usdt',$cmm_users['usdt']);//购机币
		return $this->fetch('./zizhubuy.html');
	} 
	...
	public function duihuan(){
		...
		if($_POST){
			...
			$data['recharge_quantity'] = $_POST['num'];
			//$data['transaction_password'] = $_POST['jymm'];
			$data['user_id'] = $_POST['userid'];
			$cmm_users=Db::table('cmm_users')->where('id',$data['user_id'])->setDec('usdt', $data['recharge_quantity']);
			$cmm_users=Db::table('cmm_users')->where('id',$data['user_id'])->setInc('money_wallet', $data['recharge_quantity']);
			if($cmm_users){
				echo "提交成功";
			}else{
				echo "提交失败！";
...
/* 		//释放
					...
					if($userInfo['release_wallet']>0 && \think\Session::get('CmmUser_.release_go')){
						...
						if(!$info){
							...
							if($add){
								...
								//新增可售额度钱包
								$uid = \think\Session::get('CmmUser_.id') ;
								$user->userSetInc(array('id'=> $uid), $field='ore_wallet', $money);
	...
	public function sellOrderDooCmm()
	{
		...
		if (request()->isAjax()){
			...
			if($umoney < $buyInfo['amount']){

				return json(['s'=>'POW额度不足']);
			}

			$buywhere['id'] = $buyInfo['uid'];
			$buyuserinof = new \Org\service\UsersService();
			$buyusermark = $buyuserinof->userInfo($buywhere);	
			
			$buyphone = $buyusermark['myphone'];//买家手机号

			//print_r($buyusermark);die;
			...
			if($add){
				/************这里加短信**************/
				$this->buyMsm($buyphone, $buyInfo['orderid'],3);
