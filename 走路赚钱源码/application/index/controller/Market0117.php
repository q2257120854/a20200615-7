		...
		if (request()->isAjax()){
			...
			if($save){				
				...
				if($minfo_){
					...
					//echo $buys_1.'---'.$buys_2.'---'.$buys_3;die;
					
					//if($order['amount']>=$buys_1 && $buys_1>0 && $member['rank']==1){
					if($order['amount']>=$buys_1 && $order['amount'] < $buys_2 && $member['rank']==1){
					...
					//}elseif($order['amount']>=$buys_2 && $buys_2>0 && $member['rank']==2){
					}elseif($order['amount']>=$buys_2 && $buys_2>0 && $member['rank']==1){
						//赠送矿机等级
						$grade = 2 ;	
						$g_goods = $goods_2 ;
						//赠送购机币数量
						$g_coin = 0;					
					}elseif($order['amount']>=$buys_3 && $buys_3>0 && $member['rank']==2){
						//赠送矿机等级
						$grade = 3 ;	
						$g_goods = $goods_3 ;
						//赠送购机币数量
					...
					if($grade && $g_goods>=1){
						//会员升级
						$user->userSetInc(array('id'=>$buy_id), $field='rank', 1);
						//赠送矿机
						//送矿机
					...
					if($zj_parent){
						...
						//直推奖
						if($member['refereeid'] && $member['rank']==3){
