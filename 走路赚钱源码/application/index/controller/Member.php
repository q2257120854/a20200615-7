...
class Member  extends Cmmcom
{
			...
			if($count<$days){
				...
				while($day_nume>0){
					...
					if($num>=0){
						...
						//加到钱包
						$user->userSetInc(array('id'=>\think\Session::get('CmmUser_.id')), $field='ore_wallet', $money_coin);
	...
	public function CreateStr($code, $path_new=''){
		...
		// 白色背景和蓝色文本
		$black = ImageColorAllocate($im, 33,150,243); 
	...
	public function CreateQrcode($qrcode, $path_new){
		...
		imagecopymerge($bigImg, $qCodeImg, 230, 800, 00,0, 280,280, 100);		 
	...
	public function StrQrcode($path_new, $Strpath, $path_pic){
		...
		list($qCodeWidth, $qCodeHight, $qCodeType) = getimagesize($qCodePath);
		imagecopymerge($bigImg, $qCodeImg, 205, 1100, 0, 0, 320,80, 100);		 
		list($bigWidth, $bigHight, $bigType) = getimagesize($bigImgPath); 
