			...
			if($save){				
				...
				if($minfo_){
					...
					if($zj_parent){
						...
						$reward_s = 0;
						//print_r($zj_parent);die;
						if($member['rank']==$uGrade_1){
							$reward_s = $uReward_1;
							$num = $uGrade_1;
						}elseif($member['rank']==$uGrade_2){
							$reward_s = $uReward_2;
							$num = $uGrade_2;
						}elseif($member['rank']==$uGrade_3){
							$reward_s = $uReward_3;
							$num = $uGrade_3;
						}

						$reward_s = $reward_s;							
						$uids = $zj_parent['id'];//1代
						$parentid = $zj_parent['refereeid'];//2代
						$towparent = $user->userInfo(array('id'=>$parentid));
						if($towparent){
							$threeparent = $user->userInfo(array('id'=>$towparent['refereeid']));
						}
						
						$reward_s = explode("," , $reward_s);
						if($num == 1){
							if($reward_s[0] && $uids){
								$money = $reward_s[0] * $order['amount'] / 100;
								//奖励记录
								$data = array();
								$data['explain'] = '1代团队奖'.$reward_s[0].'%';//说明
								$data['num'] =  $money;//金额
								$data['tyle'] = 2;
								$data['uid'] =  $uids;
						...
						//		$save = $user-> userSetInc(array('id'=>$uids), $field='forepart_money', $money);
							...
							if($reward_s[1] && $parentid){
								$money = $reward_s[1] * $order['amount'] / 100;
								//奖励记录
								$data = array();
								$data['explain'] = '2代团队奖'.$reward_s[1].'%';//说明
								$data['num'] =  $money;//金额
								$data['tyle'] = 2;
								$data['uid'] =  $parentid;
								$bonus = new \Org\service\BonusLogService();
								$addBonus = $bonus->bonuslogAdd($data);
								//更新购机币
								$save = $user-> userSetInc(array('id'=>$parentid), $field='money_wallet', $money);
								//更新POW额度
						//		$save = $user-> userSetInc(array('id'=>$parentid), $field='forepart_money', $money);
						...
						//		$save = $user-> userSetInc(array('id'=>$uids), $field='forepart_money', $money);
							...
							if($reward_s[2] && $threeparent['id']){
								$money = $reward_s[2] * $order['amount'] / 100;

								//奖励记录
								$data = array();
								$data['explain'] = '3代团队奖'.$reward_s[2].'%';//说明
								$data['num'] =  $money;//金额
								$data['tyle'] = 2;
								$data['uid'] =  $threeparent['id'];
								$bonus = new \Org\service\BonusLogService();
								$addBonus = $bonus->bonuslogAdd($data);
								//更新购机币
								$save = $user-> userSetInc(array('id'=>$threeparent['id']), $field='money_wallet', $money);
								//更新POW额度
						//		$save = $user-> userSetInc(array('id'=>$threeparent['id']), $field='forepart_money', $money);
