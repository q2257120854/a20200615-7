...
class cnzyzf
{
    ...
    function GetCode($order, $payment) 
	{
		...
		$pay_bankcode = ""; 
		$pay_notifyurl = $cfg_basehost.$this->pay_notifyurl;//."&code=".$payment['code'];
		
		$pay_callbackurl = $cfg_basehost.$this->callbackurl;    //."?do=>ok&oid=>".$pay_orderid;
        ...
        $def_url  = '<form style="text-align:center;" method="post" action="'.$this->gateway.'"target="_blank">';  
		 foreach ($requestarray as $key => $val) {
    ...
    */
    function respond() {
		...
		if(preg_match ("/S-P[0-9]+RN[0-9]/",$orderid)) {
			...
			//获取订单信息，检查订单的有效性
			if(!is_array($row)||$row['sta']==2)  return $msg = "您已成功支付!"; //20170120 ShowMsg("您已成功支付!","/member/",0,2000);exit;
    ...
    function success_mem($order_sn,$pname,$product,$pid) {
        ...
        if($product=="card")
        {
            ...
            {
                $this->log_result("verify_success,订单号:".$order_sn); //将验证结果存入文件
           //                           ShowMsg($oldinf,"/member/buy.php",0,2000); exit;
  return $oldinf;
            } else {
                $this->log_result ("verify_failed,订单号:".$order_sn);//将验证结果存入文件
                return "支付失败！";
        ...
        /* 改变会员订单状态_支付成功 */
            ...
            {
                $this->log_result("verify_success,订单号:".$order_sn); //将验证结果存入文件
                     //20170115               ShowMsg("会员升级成功！","/member/",0,2000);exit;
								    return "会员升级成功！";
            } else {
                $this->log_result ("verify_failed,订单号:".$order_sn);//将验证结果存入文件
                return "会员升级失败！";
        ...
        }    else if($product=="payjb")
        {
            ...
            {
                $this->log_result("verify_success,订单号:".$order_sn); //将验证结果存入文件
                      //20170115     ShowMsg($oldinf,"/member/buy.php",0,2000); exit;
						  return $oldinf;
