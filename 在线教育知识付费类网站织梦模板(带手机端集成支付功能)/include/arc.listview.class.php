...
class ListView
{
    ...
    function CountRecord()
    {
        ...
        if($this->TotalResult==-1)
        {
			...
			//课程分类
			if($this->TypeLink->TypeInfos['channeltype'] == 18){												  				  
				$tname = $this->TypeLink->TypeInfos['typename'];				 				  
				$cquery = "SELECT count(id) AS dd FROM `#@__arctype` arc WHERE reid = '18' and fenlei = '$tname' ";				
			}
    ...
    function MakeHtml($startpage=1, $makepagesize=0, $isremote=0)
    {
        ...
        if(!$GLOBALS['rewrite_open'] || $GLOBALS['rewrite_open']==2) CreateDir(MfTypedir($this->Fields['typedir']));
        $murl = '';
        if($makepagesize > 0)
        {
    ...
    $imgwidth=120,$imgheight=90,$listtype="all",$orderby="default",$innertext="",$tablewidth="100",$ismake=1,$orderWay='desc')
    {
        ...
        else
        {
			...
			if($this->TypeLink->TypeInfos['channeltype'] == 18){
				$tname = $this->TypeLink->TypeInfos['typename'];
				$query = "SELECT id FROM `#@__arctype` arc WHERE reid = '18' and fenlei = '$tname' order by sortrank asc  LIMIT $limitstart,$row ";				
			}
			...
			//课程分类		   
			 if($this->TypeLink->TypeInfos['channeltype'] == 18){
				$tname = $this->TypeLink->TypeInfos['typename'];
				$query = "select *,SUM(a.click) as click from #@__arctype t left join #@__archives a on a.typeid= t.id  where t.id in($idstr)  and t.fenlei = '$tname'  group by t.id  order by t.sortrank asc ";		
			}
    ...
    function GetPageListST($list_len,$listitem="index,end,pre,next,pageno")
    {
        ...
        if($prepagenum==1)
        {
        $prepage.="<a href=\"".$typedir."/\" rel='nofollow'>上一页</a>\r\n";
        }
         else
        {
        $prepage.="<a href=\"".str_replace("{page}",$prepagenum,$tnamerule)."\" rel='nofollow'>上一页</a>\r\n";
        }
         $indexpage="<a href=\"".$typedir."/\" rel='nofollow'>首页</a>\r\n";
        }

        //下一页,未页的链接
        if($this->PageNo!=$totalpage && $totalpage>1)
        {
            $nextpage.="<a href='".str_replace("{page}",$nextpagenum,$tnamerule)."' rel='nofollow'>下一页</a>\r\n";
            $endpage="<a href='".str_replace("{page}",$totalpage,$tnamerule)."' rel='nofollow'>末页</a>\r\n";
        }
        else
        {
            $endpage="<a href='".str_replace("{page}",$totalpage,$tnamerule)."' rel='nofollow'>末页</a>\r\n";
        ...
        for($j;$j<=$total_list;$j++)
        {
            ...
            else
            {
               if($j==1)
        {
        $listdd.="<a href=\"".$typedir."/\" rel='nofollow'>".$j."</a>\r\n";
         }
         else
         {
          $listdd.="<a href=\"".str_replace("{page}",$j,$tnamerule)."\" rel='nofollow'>".$j."</a>\r\n";
          }
          }
        }
        $plist = '';
       ...
       //170104 if(preg_match('/option/i', $listitem)) $plist .= $optionlist;
  ...
  //170104      if(preg_match('/info/i', $listitem)) $plist .= $maininfo;
