...
class SearchView
{
     ...
     * @return    string
     */
    function __construct($typeid,$keyword,$orderby,$channeltype,
    $searchtype='',$starttime=0,$upagesize=20,$kwtype=1,$mid=0)
    {
        ...
        if ($channeltype == 17){$tempfile = $GLOBALS['cfg_basedir'].$GLOBALS['cfg_templets_dir']."/".$GLOBALS['cfg_df_style']."/searchvideo.htm";} 
        else if ($channeltype == 18){$tempfile = $GLOBALS['cfg_basedir'].$GLOBALS['cfg_templets_dir']."/".$GLOBALS['cfg_df_style']."/searchdoc.htm";} 
        else if ($channeltype == 6){$tempfile = $GLOBALS['cfg_basedir'].$GLOBALS['cfg_templets_dir']."/".$GLOBALS['cfg_df_style']."/searchshop.htm";} 
    ...
    //php4构造函数
    function SearchView($typeid,$keyword,$orderby,$channeltype,
    $searchtype="",$starttime=0,$upagesize=20,$kwtype=1,$mid=0)
    {
        $this->__construct($typeid,$keyword,$orderby,$channeltype,$searchtype,$starttime,$upagesize,$kwtype,$mid);
    }
 ...
 //九戒织梦 搜索栏目 2019年2月24日16:15:41 
 /*
$karr = str_split_unicode($this->Keyword);
		$likesql = '';
		foreach($karr as $k=>$v){
			if(empty($likesql)){
				$likesql  = " typename like '%".$v."%' ";
			}else{
				$likesql  .= " or typename like '%".$v."%' ";
			}
			
}
$cquery = "select * from #@__arctype where ($likesql) and reid = '18' order by id desc "; 
*/
$karr = str_split_unicode($this->Keyword);
		$likesql = '';
		$ifsql = '';
		foreach($karr as $k=>$v){
			if(empty($likesql)){
				$ifsql = " IF(LOCATE('$v', typename), 1, 0)  ";
				$likesql  = " typename like '%".$v."%' ";
			}else{
				$ifsql .= " +  IF(LOCATE('$v', typename), 1, 0)  ";
				$likesql  .= " or typename like '%".$v."%' ";
...
$cquery = "select *,($ifsql ) as score  from #@__arctype where ($likesql) and reid = '18' order by score desc , id desc "; 
...
{                    
    ...
    $imgwidth=120,$imgheight=90,$channeltype,$orderby="default",$innertext="",$tablewidth="100")
    {
...
$query = "select *,($ifsql ) as score  from #@__arctype where ($likesql) and reid = '18' order by score desc , id desc  LIMIT $limitstart,$row"; 
//var_dump($query);
        ...
        for($i=0;$i<$row;$i++)
        {
            ...
            for($j=0;$j<$col;$j++)
            {
                ...
                if($row = $this->dsql->GetArray("al"))
                {
                   ...
                   if ($channeltype == 17){$row["shrq"]=Search_video($row["id"],"shrq");} 
        if ($channeltype == 6){$row["tdts"]=Search_shop($row["id"],"tdts"); $row["trueprice"]=Search_shop($row["id"],"trueprice"); } 
    ...
    function GetPageListDM($list_len)
    {
        ...
        $infos = "<span class=\"pageinfo\"><strong>".$this->TotalResult."</strong>条<strong>{$totalpage}</strong>页 </span>\r\n";
        ...
        if($this->PageNo != 1)
        {
            $prepage.="<a href='".$purl."PageNo=$prepagenum' class='pre'>上一页</a>\r\n";
        //    $indexpage="<a href='".$purl."PageNo=1'>首页</a>\r\n";
        }
        else
        {
        //    $indexpage="<span  class=\"cpage\">首页</span>\r\n";
        }
        if($this->PageNo!=$totalpage && $totalpage>1)
        {
            $nextpage.="<a href='".$purl."PageNo=$nextpagenum' class='next'>下一页</a>\r\n";
         //   $endpage="<a href='".$purl."PageNo=$totalpage'>末页</a>\r\n";
        }
        else
        {
          //  $endpage="<span  class=\"cpage\">末页</span>\r\n";
        }
