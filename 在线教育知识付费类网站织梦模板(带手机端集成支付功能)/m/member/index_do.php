...
else if($fmdo=='login')
{
    ...
    if($dopost=="login")
    {
        ...
        else
        {
		    ...
		    if($_SESSION['isApp'] == 1){
			   if(!$gourl){
			   $gourl = $_SERVER['HTTP_REFERER'].'?loginback=2';
			   }else{
				$gourl = $gourl.'?loginback=5';   
			   }
            ...
            {
                ShowMsg("登录成功，正在加载中...","/",0,1000);
            }
            else
            {
                $gourl = str_replace('^','&',$gourl);
                ShowMsg("登录成功，正在加载中...",$gourl,0,2000);
            }
            exit();
        }
    }
    ...
    else if($dopost=="exit")
    {
        ...
        #/aip}}
		if($_SESSION['isApp'] == 1){
		    $gourl = '/member/?loginback=2';
			ShowMsg("您已退出当前账号！",$gourl,0,1000);
		}else{
			ShowMsg("您已退出当前账号！","/",0,1000);
		}
