...
require_once("inc/inc_catalog_options.php");
...
// 反向转换一下到 文本框	$addRow['body'] =   str_replace("<p></p>","\r\n",$addRow['body']);;
 $addRow['body']  = str_replace("\"/uploads/","\"http://www.duoweizi.net/uploads/",$addRow['body']);
   include("templets/article_edit.htm");
    exit();
}
...
{
    include('inc/archives_check_edit.php');
    ...
    if(!empty($dede_addonfields))
    {
        ...
        if (empty($dede_fieldshash) || $dede_fieldshash != md5($dede_addonfields.$cfg_cookie_encode))
        {
		   ...
		   if(strlen($title) < 10 || strlen($body) < 10)
        {
            ShowMsg('标题或内容太短！', '-1');
            exit();
        }
...
ShowMsg("成功更改文章", "content_list.php?channelid=19");
exit;
    //返回成功信息
    $msg = "　　请选择你的后续操作：
