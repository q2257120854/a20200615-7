...

{
    ...
    $dsql->ExecuteNoneQuery("UPDATE #@__member_stow SET `type`='bad' WHERE aid='$aid' AND mid='".$cfg_ml->M_ID."';");
...

{
    ...
    $ENV_GOBACK_URL = empty($_COOKIE['ENV_GOBACK_URL']) ? "xihuan.php" : $_COOKIE['ENV_GOBACK_URL'];

    $dsql->ExecuteNoneQuery("UPDATE #@__member_xihuan SET type='bad' WHERE aid='$aid' AND mid='".$cfg_ml->M_ID."' AND type='$type';");
    ...
    ShowMsg("成功删除一条喜欢记录！",$ENV_GOBACK_URL);
...

{
    ...
    include_once("inc/inc_batchup.php");
...
}else if($dopost == 'viewType'){
	//九戒织梦 收藏的栏目
	$url = GetTypeDeurl($tid);
	header("location: ".$url);
	exit();
