...
if($dopost == "bindset")
{
	...
	$tpl = "templets/bindset_weixin.htm";
	...
	$gRow = $dsql->GetOne("SELECT * FROM #@__member_login where mid = {$row['mid']}");
	if(!empty($gRow))
	{
	...
	$inQuery = "INSERT INTO `#@__member` (`mtype` ,`userid` ,`pwd` ,`uname`  ,`rank` ,
        `matt` ,`face`,`email`,`jointime` ,`joinip` ,`logintime` ,`loginip`,`sex`,`qq` )
       VALUES ('个人','$newUserName','$newUserPwd' ,'$uname','10','0','$qqRow[face]','$newUserEmail','$jointime','$joinip','$logintime','$loginip','$newSex','$qq'); "; 
