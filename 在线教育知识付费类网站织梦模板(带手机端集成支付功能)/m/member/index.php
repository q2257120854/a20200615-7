...
/*if($hnbkjf['rank'] > 10 && $hnbkjf['exptime'] == 0)
{
$dsql->ExecuteNoneQuery("UPDATE `#@__member` SET rank='10',uptime='0' WHERE mid='".$cfg_ml->M_ID."'  ");
...
if($uid=='')
{
    ...
    if(!$cfg_ml->IsLogin())
    {
        include_once("templets/login.htm");
    }
    else
    {
...
//ceil((time() - $row['uptime'])/3600/24)+1;
        ...
        {
            $cfg_ml->fields['face']=($cfg_ml->fields['sex']=='女')? '/uploads/dfboy.png' : '/uploads/dfboy.png';
        }
...
exit;  
    ...
    require_once('inc/config_space.php');
    if($action == '')
    {
        include_once(DEDEINC."/channelunit.func.php");
        $dpl = new DedeTemplate();
        $tplfile = "space/{$_vars['spacestyle']}/index.htm";
    ...
    {
        require_once('inc/space_action.php');
        exit();
    }
}