    ...
    if($dopost=='regbase')
    {
     ...
     /*   $svali = GetCkVdValue();
		...
		*/
        ...
        $userid = trim($userid);
			if(preg_match("/1[3456789]{1}\d{9}$/",$userid)){}else{  ShowMsg('手机号不正确', '-1');exit;}
        $pwd = trim($userpwd);
        $pwdc = trim($userpwdok);
		$uname = substr_replace($userid,'****',3,4);
		//echo $userid;
        ...
        {
            ShowMsg($userid."你的用户名或密码过短，不允许注册！".$pwd,"-1");
            exit();
        }
       /* if($pwdc != $pwd)
        {
		...
		*/
        ...
        if(is_array($row))
        {
            ShowMsg("你指定的手机号 {$userid} 已存在，请使用别的手机号！", "-1");
            exit();
        }
        if($safequestion==0)
        {
...
if(empty($tjrmid )){
	$tjrmid = 0;
        ...
        `matt`, `spacesta` ,`face`,`safequestion`,`safeanswer` ,`jointime` ,`joinip` ,`logintime` ,`loginip`,`tjrmid`)
       VALUES ('$mtype','$userid','$pwd','$uname','$sex','10','$dfmoney','$email','$dfscores',
       '0','$spaceSta','/uploads/dfboy.png','$safequestion','$safeanswer','$jointime','$joinip','$logintime','$loginip','$tjrmid'); ";
    ...
    exit();
                require_once("templets/reg-new3.htm");
                exit;
            } 
        } else {
    ...
    require_once("templets/reg-new.htm");
} else {
    if(!$cfg_ml->IsLogin())
    {
    ...
    exit();
    require_once("templets/reg-new2.htm");
}