...
if(!isset($dopost) || empty($dopost)){
       //确认用户登录信息
        ...
        else
        {
            ...
            $userid = $cfg_ml->M_ID;
		        ...
		        $shouhuo = $dsql->GetOne("SELECT m.email,mm.uname,mm.qq,mm.tel,mm.address,mm.youbian FROM #@__member m
        LEFT JOIN #@__member_person mm ON m.mid=mm.mid WHERE m.mid='".$cfg_ml->M_ID."'");    
    ...
    */
    ...
    if(!isset($do) || empty($do))
    {
        ...
        $dtp->LoadTemplate(_PLUS_TPL_.'/carbuyaction_m.htm');
        $dtp->Display();
        exit();
    } else if ($do == 'clickout')
    {
    ...
    */
        ...
        $dtp->Assign('carts',$carts);
        $dtp->LoadTemplate(_PLUS_TPL_.'/shops_action_payment_m.htm');
        $dtp->Display();
        exit();
    }
} else if ($dopost == 'memclickout')
{
    ...
    if(is_array($row)){
        ...
        $rs = $dsql->GetOne("SELECT * FROM `#@__payment` WHERE id='{$row['paytype']}' ");
 	    $rowdz=$dsql->GetOne("SELECT * FROM `#@__shops_userinfo` WHERE oid='$oid' ");
 }
    if(is_array($rowdz)){
     $postname=$rowdz['consignee'];
	 $tel=$rowdz['tel'];
	 $address=$rowdz['address'];
    }
	$rs['code'] = isset($rs['code'])? preg_replace("#[^0-9a-z_\-]+#i", "", $rs['code']) : "";
	if(empty($rs['code']) OR !file_exists(DEDEINC.'/payment/'.$rs['code'].'m.php'))
	{
		exit("Error:payment is not exsits!");
	}
    
    require_once DEDEINC.'/payment/'.$rs['code'].'m.php';
    $pay = new $rs['code'];
    $payment="";
...
{
    $write_list = array('alipay', 'alipaym','bank', 'cod', 'yeepay','chinabank','cnzyzf');
    $code = isset($code) && (!empty($code))? $code : $_SESSION['code'];
        
    //如果当前 $_SESSION['code']中取到的值为空的话，那就说明应该是中银支付的接口， 表示在第一次进行调用的时候使用的
   if($code==""||empty($code)){
        $code="cnzyzf";
        require_once DEDEINC.'/payment/'.$code.'m.php';
        $pay = new $code;
        $msg=$pay->respond();
        if($code=="cnzyzf"){
             echo(OK);        
        ...
        exit();  
   }else if (in_array($code, $write_list))      //正常情况
    {
        require_once DEDEINC.'/payment/'.$code.'m.php';
        $pay = new $code;
        $msg=$pay->respond();
    	...
    	file_put_contents ( "runinfo.txt", "44.Error:File Type Can\'t Recognized!\n", FILE_APPEND ); // 将字符串写入文件
