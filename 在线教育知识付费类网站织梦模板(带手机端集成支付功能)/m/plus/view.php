...
{         $keywords =  $arc->Fields["keywords"];
    ...
    */                        
    ...
    if(($needRank>1 /*&& $cfg_ml->M_Rank != $needRank */&& $arc->Fields['mid']!=$cfg_ml->M_ID))
    {
      ...
      $gmurl = "/member/buy.php\"  onclick=\"alert('未登陆或权限不足,请先升级');\"";
   if ($cfg_ml->M_Rank != $needRank){ 
   $v1 = $dsql->GetOne("SELECT * FROM `#@__addonvideo` WHERE `aid`='".$arcID."'"); 
   if(strpos($_SERVER['HTTP_USER_AGENT'],'APICloud') !== false || $_SESSION['isApp'] == 1){
	 $needgo = 1;
	 include_once(DEDETEMPLATE.'/plus/app_nianfei_m.htm');exit;
   }
   include_once(DEDETEMPLATE.'/plus/view_nianfei_m.htm');exit;
   }
	else if ($arc->Fields['typeid'] == 3 ){  include_once(DEDETEMPLATE.'/plus/view_msg3_m.htm');exit;}
	 /*  else {  include_once(DEDETEMPLATE.'/plus/view_msg_m.htm');}
        exit();*/
    }
...
if ($arc->Fields['typeid'] == '1'){  $v1 = $dsql->GetOne("SELECT * FROM `#@__addonvideo` WHERE `aid`='".$arcID."'");include_once(DEDETEMPLATE.'/plus/view_msg1_m.htm');}
	   else if ( $arc->Fields['typeid'] == '3'){  $v1 = $dsql->GetOne("SELECT `body`,`size` FROM `#@__addondoc` WHERE `aid`='".$arcID."'");include_once(DEDETEMPLATE.'/plus/view_msg3_m.htm');}
	   else {  include_once(DEDETEMPLATE.'/plus/view_msg_m.htm');}
                    $arc->Close();
                    exit();
            }
 ...
 echo "</script>";  exit; 
  ...
  if ($arc->Fields['typeid'] == '1'){  $v1 = $dsql->GetOne("SELECT * FROM `#@__addonvideo` WHERE `aid`='".$arcID."'");include_once(DEDETEMPLATE.'/plus/view_msg1_m.htm');}
	   else if ($arc->Fields['typeid'] == '3'){  $v1 = $dsql->GetOne("SELECT * FROM `#@__addondoc` WHERE `aid`='".$arcID."'"); include_once(DEDETEMPLATE.'/plus/view_msg3_m.htm');}
	   else {  include_once(DEDETEMPLATE.'/plus/view_msg_m.htm');}      
	             $arc->Close();
                exit();
            }
