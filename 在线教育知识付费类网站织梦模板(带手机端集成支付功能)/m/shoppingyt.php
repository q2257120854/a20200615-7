...
if($action=='goodfb')
{
    AjaxHead();
    $fid = intval($fid);
    $dsql->ExecuteNoneQuery("UPDATE `#@__shopping` SET good = good+1 WHERE id='$fid' ");
    $row = $dsql->GetOne("SELECT good FROM `#@__shopping` WHERE id='$fid' ");
    echo "{$row['good']}";
    exit();
}
else if($action=='badfb')
{
    AjaxHead();
    $fid = intval($fid);
    $dsql->ExecuteNoneQuery("UPDATE `#@__shopping` SET bad = bad+1 WHERE id='$fid' ");
    $row = $dsql->GetOne("SELECT bad FROM `#@__shopping` WHERE id='$fid' ");
...
{
	  $goods = $dsql->GetOne("SELECT COUNT(*) AS dd FROM `#@__shopping` WHERE aid='$aid' AND ischeck='1' And ftype like 'good' ");
	    $feedbacks = $dsql->GetOne("SELECT COUNT(*) AS dd FROM `#@__shopping` WHERE aid='$aid' AND ischeck='1' And ftype like 'feedback'  ");
	    $bads = $dsql->GetOne("SELECT COUNT(*) AS dd FROM `#@__shopping` WHERE aid='$aid' AND ischeck='1' And ftype like 'bad' ");
  	    $youtus = $dsql->GetOne("SELECT COUNT(*) AS dd FROM `#@__shopping` WHERE aid='$aid' AND ischeck='1' And imgurls <> '' ");
  //读取文档信息
    ...
    if(empty($ftype) || ($ftype!='good' && $ftype!='bad' && $ftype!='feedback' && $ftype!='youtu'))
    {
        $ftype = '';
    }
		if ($ftype == 'youtu'){$wquery = $ftype!='' ? " And imgurls != '' " : '';}else{$wquery = $ftype!='' ? " And ftype like '$ftype' " : '';}
  //  $wquery = $ftype!='' ? " And ftype like '$ftype' " : '';
    ...
    $dlist->SetTemplate(DEDETEMPLATE.'/plus/shoppingyt_m.htm');
...
else if($action=='quote')
{
    ...
    $row = $dsql->GetOne("SELECT * FROM `#@__shopping` WHERE id ='$fid'");
