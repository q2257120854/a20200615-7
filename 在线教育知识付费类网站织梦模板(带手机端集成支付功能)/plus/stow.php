...
$addtime = time();
	...
	if(!is_array ($row))
	{
		$dsql->ExecuteNoneQuery("INSERT INTO `#@__member_stow`(mid,aid,title,addtime,type) VALUES ('".$ml->M_ID."','$aid','".addslashes($arctitle)."','$addtime','good'); ");
}else {if ($row['type'] != 'good'){ $dsql->ExecuteNoneQuery("UPDATE #@__member_stow SET `type`='good' WHERE `id`='".$row['id']."'");
}
	else{ShowMsg('已经收藏过','view.php?aid='.$aid);exit;}
}
