...

{
    ...
    $query = "DELETE FROM `#@__shouhou` WHERE id IN($ids) ";
...

{
    ...
    $dsql->SetQuery("SELECT ip FROM `#@__shouhou` WHERE id IN ($ids) ");
...

    {
        ...
        $query = "DELETE FROM `#@__shouhou` WHERE $ips ";
...

{
        ...
        $query = "UPDATE `#@__shouhou` SET ischeck=1 WHERE id IN($ids) ";
...

{
    ...
    $querystring = "SELECT * FROM `#@__shouhou` WHERE msg LIKE '%$keyword%' $addsql ORDER BY dtime DESC";
    ...
    $dlist->SetTemplate(DEDEADMIN.'/templets/shouhou_main.htm');
