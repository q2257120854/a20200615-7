...
$row = $dsql->GetOne("SELECT pid,dprice,state,kuaidi,kuaididan FROM #@__shops_orders WHERE oid='$oid'");

if($comtype == 'kuaidi')

    { 

    $kuaidi = cn_substrR(TrimMsg($kuaidi), 1000);



 $rs = 	$dsql->ExecuteNoneQuery("UPDATE `#@__shops_orders` SET kuaidi='$kuaidi',kuaididan='$kuaididan',state='2' WHERE oid='$oid' ");
