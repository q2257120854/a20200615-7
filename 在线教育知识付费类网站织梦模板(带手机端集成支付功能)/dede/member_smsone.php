 ...
 * @version        $Id: member_smsone.php 1 11:24 2010年7月20日Z tianya $
...

{
   ...
   /* if(!is_array($row))
	...
	//PutCookie('mobile_code', $vacode, 3600 * 24, '/');
	...
	$infos=$row['value'].','.$row_pwd['value'].','.$msgtoid;
 ...
 ShowMsg('短信已成功发送','member_smsone.php');
   ...
   /* $inquery = "INSERT INTO `#@__member_pms` (`floginid`,`fromid`,`toid`,`tologinid`,`folder`,`subject`,`sendtime`,`writetime`,`hasview`,`isadmin`,`message`)
      ...
      VALUES ('sms','$fromid','{$row['mid']}','{$row['userid']}','inbox','$subject','$sendtime','$writetime','0','0','$message'); ";
...
require_once(DEDEADMIN."/templets/member_smsone.htm");