...
$nianping = $dsql->GetOne("SELECT id FROM `#@__shopping` WHERE aid='".$aid."' and mid='".$cfg_ml->M_ID."'; ");

//$rowhnbk=$dsql->GetOne("SELECT  * FROM `#@__member_person` WHERE mid='".$cfg_ml->M_ID."'");

 $lll = $dsql->GetOne("SELECT litpic,title,money,typeid,pubdate FROM `#@__archives` WHERE id='$aid' ");

 if(is_array($nianping))
...
$ischeck = $cfg_feedbackcheck=='Y' ? 0 : 1;
 ...
 //保存评论内容
...

    {
        ...
        $tpl = dirname(__FILE__)."/templets/nianping.htm";
