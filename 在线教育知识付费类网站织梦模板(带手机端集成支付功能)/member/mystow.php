...
if($rank == 'top'){
    ...
    $sql = "SELECT s.*,t.*,arc.litpic,arc.money,arc.title as title FROM `#@__member_stow` AS s left join `#@__member_stowtype` AS t on t.stowname=s.type  left join `#@__archives` AS arc on arc.id=s.aid  where s.mid='".$cfg_ml->M_ID."'   AND s.type ='good' order by s.addtime desc";
    $tpl = 'mystow';
}
